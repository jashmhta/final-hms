...
class E2EHealthcareTestCase(LiveServerTestCase):
    ...
    def navigate_to_patient_registration(self):
        ...
        self.driver.find_element(
            By.XPATH, '//button[contains(text(), "Register New Patient")]'
        ).click()
    ...
    def fill_patient_registration_form(self, patient_data):
        ...
        # Medical information
        self.driver.find_element(By.ID, "id_blood_type").send_keys(
        ...
        # Emergency contact
        self.driver.find_element(By.ID, "id_emergency_contact_name").send_keys(
            patient_data.get("emergency_contact_name", "")
        )
        self.driver.find_element(By.ID, "id_emergency_contact_phone").send_keys(
    ...
    def navigate_to_appointment_scheduling(self):
        ...
        self.driver.find_element(
            By.XPATH, '//a[contains(text(), "Appointments")]'
        ).click()
    ...
    def navigate_to_medical_records(self, patient_name):
        ...
        self.driver.find_element(
            By.XPATH, '//a[contains(text(), "Medical Records")]'
        ).click()
        ...
        patient_link = self.driver.find_element(
            By.XPATH, f'//a[contains(text(), "{patient_name}")]'
        )
        patient_link.click()
    ...
    def create_medical_record(self, record_data):
        ...
        self.driver.find_element(
            By.XPATH, '//button[contains(text(), "Add Medical Record")]'
        ).click()
    ...
    def create_bill(self, bill_data):
        """Create a new bill"""
        self.driver.find_element(
            By.XPATH, '//button[contains(text(), "Create Bill")]'
        ).click()
    ...
    def process_payment(self, payment_data):
        ...
        self.driver.find_element(
            By.XPATH, '//button[contains(text(), "Process Payment")]'
        ).click()
...
class TestPatientRegistrationJourney(E2EHealthcareTestCase):
    ...
    def test_complete_patient_registration_flow(self):
        ...
        self.assertTrue(
            Patient.objects.filter(
                first_name="John", last_name="Doe", email="patient.test@example.com"
            ).exists()
        )
        ...
        audit_logs = AuditLog.objects.filter(
            model_name="Patient", object_id=str(patient.id), action="CREATE"
        )
    ...
    def test_patient_registration_validation_errors(self):
        ...
        self.assertGreater(
            len(error_messages), 0, "No validation errors shown for empty form"
        )
...
class TestAppointmentSchedulingJourney(E2EHealthcareTestCase):
    ...
    def test_complete_appointment_scheduling_flow(self):
        ...
        appointment_data = {
            ...
            "scheduled_date": (datetime.now() + timedelta(days=1)).strftime(
                "%Y-%m-%dT%H:%M"
            ),
            "duration": 30,
        ...
        self.assertTrue(
            Appointment.objects.filter(
                patient=patient, doctor=doctor, appointment_type="CONSULTATION"
            ).exists()
        )
        ...
        self.assertTrue(
            AppointmentReminder.objects.filter(appointment=appointment).exists()
        )

        # Measure journey completion time
        journey_time = time.time() - start_time
        self.assertLess(
            journey_time, 45, "Appointment scheduling journey took too long"
        )
    ...
    def test_appointment_conflict_prevention(self):
        ...
        conflict_data = {
            ...
            "doctor_name": f"{doctor.first_name} {doctor.last_name}",
            "appointment_type": "CONSULTATION",
            "scheduled_date": (existing_time + timedelta(minutes=30)).strftime(
                "%Y-%m-%dT%H:%M"
            ),
            "duration": 60,
    ...
    def test_appointment_cancellation_flow(self):
        ...
        cancel_button = self.driver.find_element(
            By.XPATH, f'//button[contains(@data-appointment-id, "{appointment.id}")]'
        )
        cancel_button.click()
...
class TestMedicalRecordManagementJourney(E2EHealthcareTestCase):
    ...
    def test_complete_medical_record_workflow(self):
        ...
        medical_record = MedicalRecord.objects.get(
            patient=patient, chief_complaint="Patient complains of chest pain"
        )
        from core.models import AuditLog
        ...
        self.assertNotEqual(
            medical_record._chief_complaint, medical_record.chief_complaint
        )

        # Measure journey completion time
        journey_time = time.time() - start_time
        self.assertLess(
            journey_time, 50, "Medical record management journey took too long"
        )

    def test_medical_record_access_control(self):
        """Test medical record access control"""
        patient = self.create_anonymized_patient()
        doctor = self.create_test_user(
            role="DOCTOR", username="doctor", email="doctor@hms.com"
        )
        nurse = self.create_test_user(
            role="NURSE", username="nurse", email="nurse@hms.com"
        )
        patient_user = self.create_test_user(
            role="PATIENT", username="patient", email="patient@hms.com"
        )
    ...
    def test_medical_record_search_and_filter(self):
        ...
        self.wait.until(
            EC.presence_of_element_located((By.CLASS_NAME, "medical-record-item"))
        )
...
class TestBillingAndPaymentJourney(E2EHealthcareTestCase):
    ...
    def test_complete_billing_workflow(self):
        ...
        self.assertTrue(
            Bill.objects.filter(patient=patient, bill_number="BILL-001").exists()
        )
        ...
        self.assertTrue(
            Payment.objects.filter(
                bill=bill, amount=Decimal("200.00"), transaction_id="TXN-001"
            ).exists()
        )
    ...
    def test_billing_error_handling(self):
        ...
        error_messages = self.driver.find_elements(By.CLASS_NAME, "errorlist")
        self.assertGreater(
            len(error_messages), 0, "No validation errors shown for invalid bill data"
        )
...
class TestEmergencyRoomJourney(E2EHealthcareTestCase):
    ...
    def test_emergency_patient_triage_flow(self):
        ...
        emergency_patient = self.create_anonymized_patient(
            first_name="Emergency", last_name="Patient", medical_record_number="ER-001"
        )
        ...
        self.driver.find_element(
            By.XPATH, '//button[contains(text(), "Start Triage")]'
        ).click()
        ...
        self.wait.until(
            EC.presence_of_element_located(
                (By.XPATH, '//button[contains(text(), "Select Patient")]')
            )
        )
        ...
        # Fill triage form
        self.driver.find_element(By.ID, "id_chief_complaint").send_keys(
            triage_data["chief_complaint"]
        )

        # Vital signs
        self.driver.find_element(By.ID, "id_blood_pressure").send_keys(
            triage_data["vital_signs"]["blood_pressure"]
        )
        self.driver.find_element(By.ID, "id_heart_rate").send_keys(
            str(triage_data["vital_signs"]["heart_rate"])
        )
        self.driver.find_element(By.ID, "id_respiratory_rate").send_keys(
            str(triage_data["vital_signs"]["respiratory_rate"])
        )
        self.driver.find_element(By.ID, "id_temperature").send_keys(
            str(triage_data["vital_signs"]["temperature"])
        )
        self.driver.find_element(By.ID, "id_oxygen_saturation").send_keys(
            str(triage_data["vital_signs"]["oxygen_saturation"])
        )
        ...
        self.assertTrue(
            ERTriage.objects.filter(
                patient=emergency_patient, triage_level="HIGH"
            ).exists()
        )

        # Check for emergency alerts
        alerts = PatientAlert.objects.filter(
            patient=emergency_patient, alert_type="EMERGENCY", severity="HIGH"
        )
    ...
    def test_emergency_department_admission_flow(self):
        ...
        # Process admission
        admission_button = self.driver.find_element(
            By.XPATH, '//button[contains(text(), "Process Admission")]'
        )
...
class TestPharmacyWorkflowJourney(E2EHealthcareTestCase):
    ...
    def test_prescription_filling_workflow(self):
        ...
        self.driver.find_element(
            By.XPATH, '//a[contains(text(), "Pending Prescriptions")]'
        ).click()
        self.wait.until(EC.url_contains("/pharmacy/prescriptions/"))

        # Select prescription to fill
        prescription_link = self.driver.find_element(
            By.XPATH, f'//a[contains(@href, "{prescription.id}")]'
        )
        prescription_link.click()

        # Fill prescription
        self.driver.find_element(
            By.XPATH, '//button[contains(text(), "Fill Prescription")]'
        ).click()
        ...
        # Process filling
        self.driver.find_element(By.ID, "id_quantity_dispensed").send_keys(
            "14"
        )  # 2 tablets x 7 days
    ...
    def test_medication_inventory_management(self):
        ...
        self.driver.find_element(
            By.XPATH, '//button[contains(text(), "Add Batch")]'
        ).click()
        ...
        self.driver.find_element(By.ID, "id_expiry_date").send_keys(
        ...
        self.assertTrue(
            MedicationBatch.objects.filter(
                medication=medication, batch_number="BATCH-002"
            ).exists()
        )
        ...
        if medication.current_stock < medication.reorder_level:
            alerts = InventoryAlert.objects.filter(
                medication=medication, alert_type="STOCK"
            )
            self.assertTrue(alerts.exists())
