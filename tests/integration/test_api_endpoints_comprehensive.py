...
class TestPatientAPIEndpoints(ComprehensiveHMSTestCase):
    ...
    def test_patient_insurance_api(self):
        ...
        self.assertTrue(InsuranceInformation.objects.filter(
            patient=patient,
            provider_name='Test Insurance'
        ).exists())
    ...
    def test_patient_api_authorization(self):
        ...
        roles_and_expected_status = [
            ...
            ('PATIENT', status.HTTP_403_FORBIDDEN),  # Patients shouldn't access other patient data
        ]
    ...
    def test_patient_api_compliance(self):
        ...
        self.assert_healthcare_compliance(response, HealthcareDataType.PATIENT_DEMOGRAPHICS)
...
class TestMedicalRecordAPIEndpoints(ComprehensiveHMSTestCase):
    ...
    def test_medical_record_create_api(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertIn('id', response.data)
        self.assertEqual(response.data['chief_complaint'], 'Patient complains of chest pain')
...
class TestAppointmentAPIEndpoints(ComprehensiveHMSTestCase):
    ...
    def test_appointment_date_filter(self):
        ...
        Appointment.objects.create(
            ...
            doctor=self.test_user,
            appointment_type='CONSULTATION',
            scheduled_date=datetime.combine(today, datetime.min.time()) + timedelta(hours=10),
            duration=30,
            status='SCHEDULED'
        )
        ...
        Appointment.objects.create(
            ...
            scheduled_date=datetime.combine(tomorrow, datetime.min.time()) + timedelta(hours=11),
            duration=30,
            status='SCHEDULED'
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']), 1)
        self.assertEqual(response.data['results'][0]['scheduled_date'].split('T')[0], today.isoformat())
    ...
    def test_appointment_cancel_api(self):
        ...
        cancel_data = {
            ...
            'cancelled_by': self.test_user.id
        }
    ...
    def test_appointment_reminder_api(self):
        ...
        reminder_data = {
            'reminder_type': 'EMAIL',
            'reminder_time': (appointment.scheduled_date - timedelta(hours=24)).isoformat(),
        ...
        self.assertTrue(AppointmentReminder.objects.filter(
    ...
    def test_appointment_calendar_api(self):
        ...
        for i in range(5):
            Appointment.objects.create(
                ...
                doctor=self.test_user,
                appointment_type='CONSULTATION',
                scheduled_date=datetime.combine(today, datetime.min.time()) + timedelta(hours=9+i),
                duration=30,
                status='SCHEDULED'
            )
...
class TestBillAPIEndpoints(ComprehensiveHMSTestCase):
    ...
    def test_bill_discount_api(self):
        ...
        self.assertTrue(BillDiscount.objects.filter(
            bill=bill,
            discount_type='PERCENTAGE',
