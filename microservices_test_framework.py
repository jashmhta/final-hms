"""
microservices_test_framework module
"""
        ...
        try:
            ...
            async with self.session.get(f"{base_url}/health") as response:
                ...
                if response.status == 200:
                    ...
                    self.logger.info(
                        f"✅ {service_name} health check passed ({response_time:.3f}s)"
                    )
                else:
            ...
            async with self.session.get(f"{base_url}/") as response:
                if response.status == 200:
                    ...
                    result.errors.append(
                        f"Root endpoint returned status {response.status}"
                    )
...
async def main():
    ...
    async with MicroservicesTestFramework() as framework:
        ...
        report_file = (
            "/home/azureuser/hms-enterprise-grade/microservices_test_report.json"
        )
