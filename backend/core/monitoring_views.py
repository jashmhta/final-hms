...
class MetricsView(View):
    ...
    def get(self, request: HttpRequest) -> JsonResponse:
        ...
        try:
            ...
            return JsonResponse(
                {"metrics": metrics_data}, status=200, content_type=CONTENT_TYPE_LATEST
            )
        except Exception as e:
        ...
        try:
            ...
            error_counts = (
                AuditLog.objects.filter(
                    created_at__gte=last_24h, action__in=["exception", "security_event"]
                )
                .values("action")
...
class MonitoringViewSet(viewsets.ViewSet):
    ...
    def error_analysis(self, request):
        ...
        error_types = (
            AuditLog.objects.filter(created_at__gte=start_time, action="exception")
        ...
        return Response(
            {
                ...
                "error_trends": [
                    {"hour": trend["hour"].isoformat(), "count": trend["count"]}
    ...
    def _get_api_metrics(self, hours: int) -> Dict[str, Any]:
        ...
        try:
            ...
            api_logs = AuditLog.objects.filter(
                created_at__gte=start_time, action__in=["request_start", "request_end"]
            )
    ...
    def _get_error_metrics(self, hours: int) -> Dict[str, Any]:
        ...
        try:
            ...
            error_logs = AuditLog.objects.filter(
                created_at__gte=start_time, action="exception"
            )
