"""
setup_database_partitioning module
"""
...
from core.database_partitioning import HealthcareDataCategory, database_partitioner
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not database_partitioner.is_partitioning_supported():
            ...
            self.stdout.write("Partitioning requires PostgreSQL 10.0 or later.")
            return

        self.stdout.write(
            self.style.SUCCESS(
                "Setting up database partitioning for healthcare datasets..."
            )
        )
        ...
        if not tables_to_partition:
            self.stdout.write(self.style.WARNING("No tables found to partition."))
            return
        ...
        for table_name in tables_to_partition:
            if table_name in database_partitioner.partition_configs:
                ...
                if migrate_data:
                    result = database_partitioner.migrate_to_partitioned_table(
                        table_name, config
                    )
                else:
    ...
    def _get_tables_to_partition(self, table_name: str, category: str) -> List[str]:
        ...
        if table_name:
            if table_name in database_partitioner.partition_configs:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Table {table_name} not found in partitioning configuration."
                    )
                )
        elif category:
    ...
    def _display_table_result(
        self, table_name: str, result: Dict[str, Any], dry_run: bool
    ):
        ...
        if result.get("migration_completed"):
            ...
            self.stdout.write(
                f'  Partitions created: {result.get("partitions_created", 0)}'
            )
    ...
    def _display_job_result(self, result: Dict[str, Any], dry_run: bool):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING("[DRY RUN] Would setup maintenance job")
            )
            return
        ...
        if result.get("job_created"):
            ...
            self.stdout.write(self.style.ERROR("  Failed to setup maintenance job"))
    ...
    def _display_summary(self, results: Dict[str, Any], dry_run: bool):
        ...
        migrated_tables = sum(
            1 for r in results.values() if r.get("migration_completed")
        )
        ...
        self.stdout.write("\nNext Steps:")
        self.stdout.write(
            "1. Monitor partition performance with: python manage.py partition_stats"
        )
        self.stdout.write(
            "2. Run maintenance tasks with: python manage.py maintain_partitions"
        )
    ...
    def _run_maintenance_tasks(self, dry_run: bool):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING("[DRY RUN] Would run maintenance tasks")
            )
            return
