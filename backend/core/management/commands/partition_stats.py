...
class Command(BaseCommand):
        ...
        try:
            with database_optimizer.get_database_connection() as connection:
                with connection.cursor() as cursor:
                    ...
                    if empty_partitions:
                        health['warnings'].append(f"Found {len(empty_partitions)} empty partitions")
                        for partition in empty_partitions:
                            health['warnings'].append(f"  Empty partition: {partition[1]}")
                    ...
                    if oversized_partitions:
                        health['warnings'].append(f"Found {len(oversized_partitions)} oversized partitions (>1GB)")
                        for partition in oversized_partitions:
                            size_gb = partition[2] / (1024 * 1024 * 1024)
                            health['warnings'].append(f"  Oversized partition: {partition[1]} ({size_gb:.1f} GB)")
                    ...
                    for partition_info in partitions_info:
                        ...
                        if partition_count > 100:
                            health['warnings'].append(f"Table {table_name} has {partition_count} partitions (consider archiving)")
    ...
    def _display_stats(self, stats: dict, detailed: bool, health_check: bool, recommendations: bool):
        ...
        if not stats.get('partitioning_enabled', False):
            self.stdout.write(self.style.ERROR('Partitioning is not enabled or supported'))
            return

        # General information
        self.stdout.write(f'Timestamp: {stats.get("timestamp", "N/A")}')
        self.stdout.write(f'Partitioning Enabled: {stats.get("partitioning_enabled", False)}')
        ...
        if stats.get('tables'):
            ...
            for table_name, table_stats in stats['tables'].items():
                ...
                if table_stats.get('error'):
                    self.stdout.write(self.style.ERROR(f'  Error: {table_stats["error"]}'))
                    continue
                ...
                self.stdout.write(f'  Partition Strategy: {table_stats.get("partition_strategy", "N/A")}')
                self.stdout.write(f'  Partition Attributes: {table_stats.get("partition_attributes", "N/A")}')
                ...
                if total_rows > 0 and partition_count > 0:
                    ...
                    self.stdout.write(f'  Avg Rows/Partition: {avg_rows_per_partition:,.0f}')
                    self.stdout.write(f'  Avg Size/Partition: {avg_size_per_partition:.1f} MB')
                ...
                if detailed and table_stats.get('partitions'):
                    ...
                    for partition in table_stats['partitions']:
                        ...
                        self.stdout.write(f'    {partition_name}: {row_count:,} rows, {size_mb:.1f} MB')
        ...
        self.stdout.write('• Monitor query performance to identify partition access patterns')
        ...
        self.stdout.write('• Setup partitioning: python manage.py setup_database_partitioning')
        self.stdout.write('• Run maintenance: python manage.py maintain_partitions')
        self.stdout.write('• Create maintenance job: python manage.py setup_database_partitioning --maintenance-only')
        self.stdout.write('• Migrate data: python manage.py setup_database_partitioning --migrate-data')