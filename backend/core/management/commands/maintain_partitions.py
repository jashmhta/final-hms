...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if table_name:
            create_result = database_partitioner.create_time_partitions(table_name, months_ahead)
    ...
    def _display_maintenance_results(self, result: dict, verbose: bool, dry_run: bool):
        ...
        self.stdout.write(f'Maintenance Timestamp: {result.get("maintenance_timestamp", "N/A")}')
        ...
        if verbose and result.get('tables_status'):
            ...
            for table_name, table_result in result['tables_status'].items():
                ...
                if table_result.get('partitions_created', 0) > 0:
                    self.stdout.write(
                        self.style.SUCCESS(f'  Created {table_result["partitions_created"]} new partitions')
                    )

                if table_result.get('partitions_archived', 0) > 0:
                    self.stdout.write(
                        self.style.SUCCESS(f'  Archived {table_result["partitions_archived"]} old partitions')
                    )
    ...
    def _show_next_maintenance_schedule(self):
        ...
        self.stdout.write(f'Next Daily Maintenance: {next_daily.strftime("%Y-%m-%d %H:%M")}')
        self.stdout.write(f'Next Weekly Maintenance: {next_weekly.strftime("%Y-%m-%d %H:%M")}')
        self.stdout.write(f'Next Monthly Maintenance: {next_monthly.strftime("%Y-%m-%d %H:%M")}')
