        ...
        for query in queries:
            ...
            if query_time > self.db_optimizer._slow_query_threshold_ms / 1000:
                self.slow_queries.append(
                    {"sql": sql, "time": query_time, "type": "slow_query"}
                )
    ...
    def _log_performance_metrics(self, request: HttpRequest) -> None:
        ...
        if self.query_count > 0:
            ...
            if self.query_count > 50:
                logger.warning(
                    f"High query count detected: {self.query_count} queries for {request.path}"
                )

            if self.query_time > 1.0:  # More than 1 second
                logger.warning(
                    f"Slow query time detected: {self.query_time:.3f}s for {request.path}"
                )

            if self.n_plus_one_detected:
                logger.warning(
                    f"Potential N+1 query pattern detected for {request.path}"
                )
    ...
    def _perform_health_check(self) -> None:
        ...
        try:
            ...
            if stats.get("slow_queries", 0) > 50:
                logger.warning(
                    f"Database health: High number of slow queries ({stats['slow_queries']})"
                )
            ...
            if int(time.time()) % 3600 == 0:  # Every hour
                ...
                logger.info(
                    f"Database optimization report generated: {len(report.get('table_analyses', {}))} tables analyzed"
                )
