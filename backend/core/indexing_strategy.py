...
class HealthcareIndexingStrategy:
    ...
    def _define_healthcare_indexes(self) -> Dict[str, List[IndexDefinition]]:
        ...
        return {
            ...
            "appointments_appointment": [
                ...
                IndexDefinition(
                    table_name="appointments_appointment",
                    column_names=["hospital_id", "appointment_date", "appointment_time"],
                    index_type=IndexType.BTREE,
                    priority=IndexPriority.MEDIUM,
                ...
                with connection.cursor() as cursor:
                    if self.db_optimizer.is_postgres:
                        ...
                        for index_info in result.current_indexes:
                            ...
                            if stats.get('scans', 0) == 0 and not index_info['is_primary']:
                        ...
                        if table_name in self.healthcare_indexes:
                            ...
                            existing_index_names = {idx['name'] for idx in result.current_indexes}
                            ...
                            for rec in recommended:
                                ...
                                if index_name not in existing_index_names:
                                    ...
                                    result.missing_indexes.append(f"{rec.column_names} ({rec.index_type.value})")
    ...
    def _calculate_optimization_score(self, analysis: IndexAnalysisResult) -> float:
        ...
        if analysis.missing_indexes:
            critical_missing = sum(1 for rec in analysis.recommended_indexes
        ...
        try:
            ...
            for table_name, analysis in all_results.items():
                ...
                summary['table_summaries'][table_name] = {
                    ...
                    'critical_issues': sum(1 for r in analysis.recommended_indexes
                                        if r.priority == IndexPriority.CRITICAL)
                }

            # Calculate overall optimization score
            if summary['total_tables_analyzed'] > 0:
                total_score = sum(analysis.optimization_score for analysis in all_results.values())
                summary['optimization_score'] = total_score / summary['total_tables_analyzed']
