...
class TallyIntegrationAdmin(admin.ModelAdmin):
    list_display = [
        'hospital', 'company_name', 'tally_server_url',
        'auto_sync_enabled', 'sync_frequency',
        'last_sync_status', 'last_sync_time', 'is_active'
    ]
    list_filter = [
        'is_active', 'auto_sync_enabled', 'sync_frequency',
        'last_sync_status'
    ]
    search_fields = [
        'hospital__name', 'company_name', 'tally_server_url'
    ]
    readonly_fields = [
        'last_sync_time', 'last_sync_status', 'sync_error_message'
    ]
    fieldsets = (
        ...
        ('Tally Configuration', {
            'fields': (
                'tally_server_url', 'company_name', 'tally_license_key'
            )
        }),
        ('Sync Settings', {
            'fields': (
    ...
    def enable_auto_sync(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{updated} integration(s) auto-sync enabled successfully.'
        )
    ...
    def disable_auto_sync(self, request, queryset):
        ...
        self.message_user(
            request,
            f'{updated} integration(s) auto-sync disabled successfully.'
        )
