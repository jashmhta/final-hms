...
class TallyIntegrationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Tally Configuration",
            {"fields": ("tally_server_url", "company_name", "tally_license_key")},
        ),
        ...
        (
            "Sync Status",
            {
                "fields": ("last_sync_time", "last_sync_status", "sync_error_message"),
                "classes": ("collapse",),
            },
        ),
    )
    ...
    def enable_auto_sync(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} integration(s) auto-sync enabled successfully."
        )
    ...
    def disable_auto_sync(self, request, queryset):
        ...
        self.message_user(
            request, f"{updated} integration(s) auto-sync disabled successfully."
        )
