...
class ConsentManagementAdmin(admin.ModelAdmin):
    ...
    """
    list_display = [
        'id', 'patient_name', 'consent_type', 'status', 'version',
        'consent_date', 'expiry_date', 'is_valid', 'created_by'
    ]
    list_filter = [
        'consent_type', 'status', 'consent_date', 'expiry_date',
        'hospital', 'created_by'
    ]
    search_fields = [
        'patient__first_name', 'patient__last_name',
        'patient__medical_record_number', 'title', 'description'
    ]
    readonly_fields = [
        'id', 'uuid', 'created_at', 'updated_at', 'revoked_date'
    ]
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': (
                'patient', 'consent_type', 'status', 'version',
        ...
        ('Consent Details', {
            'fields': (
                ...
                'consent_form_url', 'digital_signature_data'
            )
        }),
        ('Lifecycle', {
            'fields': (
                'consent_date', 'expiry_date', 'revoked_date',
                'withdrawal_method', 'is_active'
            )
        }),
        ('Witness Information', {
            'fields': (
                'witness_name', 'interpreter_used', 'interpreter_name',
                'language_preference'
            )
        }),
        ('Metadata', {
...
class ConsentAuditLogAdmin(admin.ModelAdmin):
    ...
    """
    list_display = [
        'id', 'patient_name', 'consent_title', 'action',
        'action_by', 'action_date', 'ip_address'
    ]
    list_filter = [
        'action', 'action_date', 'hospital', 'action_by'
    ]
    search_fields = [
        'patient__first_name', 'patient__last_name',
        'consent__title', 'details', 'ip_address'
    ]
    readonly_fields = [
        'id', 'uuid', 'patient', 'consent', 'action',
        'action_by', 'action_date', 'ip_address', 'user_agent'
    ]
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': (
                'patient', 'consent', 'action', 'action_by',
                'action_date', 'details'
            )
        }),
        ('Technical Details', {
...
class DataSubjectRequestAdmin(admin.ModelAdmin):
    ...
    """
    list_display = [
        'id', 'patient_name', 'request_type', 'status',
        'received_date', 'due_date', 'assigned_to', 'priority'
    ]
    list_filter = [
        'request_type', 'status', 'priority', 'received_date',
        'due_date', 'assigned_to', 'hospital'
    ]
    search_fields = [
        'patient__first_name', 'patient__last_name',
        'description', 'response_message', 'rejection_reason'
    ]
    readonly_fields = [
        'id', 'uuid', 'received_date', 'due_date', 'completed_date'
    ]
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': (
                'patient', 'request_type', 'status', 'priority',
                'description', 'scope', 'timeframe'
            )
        }),
        ('Processing', {
            'fields': (
                'assigned_to', 'completed_by', 'received_date',
                'due_date', 'completed_date'
            )
        }),
        ('Response', {
            'fields': (
                'response_data', 'response_message', 'rejection_reason'
            )
        }),
        ('Metadata', {
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'patient', 'assigned_to', 'completed_by', 'hospital'
        )
    ...
    def assign_to_compliance_officer(self, request, queryset):
        ...
        compliance_officers = User.objects.filter(
            groups__name='Compliance Officers',
            is_active=True
        )
        ...
        self.message_user(
            request,
            _(f"Assigned {updated_count} requests to {assigned_officer.get_full_name()}"),
            level='success'
        )
    ...
    def mark_as_completed(self, request, queryset):
        ...
        self.message_user(
            request,
            _(f"Marked {updated_count} requests as completed"),
            level='success'
        )
...
class DataSubjectRequestAuditAdmin(admin.ModelAdmin):
    ...
    """
    list_display = [
        'id', 'request_type', 'patient_name', 'action',
    ...
    search_fields = [
        'request__patient__first_name', 'request__patient__last_name',
        'details'
    ]
    readonly_fields = [
        'id', 'uuid', 'request', 'action', 'action_by',
        'action_date', 'details'
    ]
    date_hierarchy = 'action_date'
    ordering = ['-action_date']
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'request', 'request__patient', 'action_by'
        )
...
def get_compliance_dashboard_data():
    ...
    request_metrics = DataSubjectRequest.objects.aggregate(
        ...
        overdue_requests=Count('id', filter=Q(due_date__lt=today, status__in=['PENDING', 'IN_PROGRESS'])),
    )
