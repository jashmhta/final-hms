...
class ConsentManagementViewSet(viewsets.ModelViewSet):
    ...
    def _can_view_patient_consents(self, patient_id, user) -> bool:
        ...
        if hasattr(user, 'patient_profile') and str(user.patient_profile.id) == str(patient_id):
            return True
...
class DataSubjectRequestViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if not self._can_make_request_for_patient(patient_id, self.request.user):
            raise PermissionDenied("You do not have permission to make requests for this patient")
    ...
    def process_access_request(self, request, pk=None):
        ...
        try:
            ...
            return Response({
                'error': f'Failed to process access request: {str(e)}'
            }, status=500)
    ...
    def process_erasure_request(self, request, pk=None):
        ...
        try:
            ...
            return Response({
                'error': f'Failed to process erasure request: {str(e)}'
            }, status=500)
...
class ComplianceDashboardView(View):
    ...
    def _get_compliance_metrics(self) -> Dict:
        ...
        active_consents = ConsentManagement.objects.filter(status=ConsentStatus.ACTIVE).count()
        expired_consents = ConsentManagement.objects.filter(status=ConsentStatus.EXPIRED).count()
        revoked_consents = ConsentManagement.objects.filter(status=ConsentStatus.REVOKED).count()
        ...
        in_progress_requests = DataSubjectRequest.objects.filter(status="IN_PROGRESS").count()
        completed_requests = DataSubjectRequest.objects.filter(status="COMPLETED").count()

        # Request type breakdown
        request_types = DataSubjectRequest.objects.values('request_type').annotate(
            count=Count('id')
        ).order_by('-count')
        ...
        return {
            'consent_management': {
                ...
                'compliance_rate': round((active_consents / total_consents * 100) if total_consents > 0 else 0, 2)
            },
            ...
            'recent_activity': [
                {
                    ...
                    'action_by': activity.action_by.get_full_name() if activity.action_by else 'System',
...
class ComplianceReportView(View):
    ...
    def _generate_consent_report(self, start_date, end_date) -> Dict:
        ...
        return {
            ...
            'summary': {
                ...
                'expired_consents': consents.filter(status=ConsentStatus.EXPIRED).count(),
                'revoked_consents': consents.filter(status=ConsentStatus.REVOKED).count(),
            },
            'consent_types': list(
                consents.values('consent_type').annotate(
                    count=Count('id')
                ).order_by('-count')
            ),
            'generated_at': timezone.now().isoformat()
    ...
    def _generate_data_requests_report(self, start_date, end_date) -> Dict:
        ...
        return {
            ...
            'request_types': list(
                requests.values('request_type').annotate(
                    count=Count('id')
                ).order_by('-count')
            ),
    ...
    def _generate_comprehensive_report(self, start_date, end_date) -> Dict:
        ...
        return {
            ...
            'compliance_score': self._calculate_compliance_score(consent_report, data_requests_report),
            'recommendations': self._generate_recommendations(consent_report, data_requests_report),
            'generated_at': timezone.now().isoformat()
        }
    ...
    def _calculate_processing_times(self, requests) -> Dict:
        ...
        for request in completed_requests:
            if request.completed_date and request.received_date:
                processing_time = (request.completed_date - request.received_date).total_seconds()
    ...
    def _calculate_compliance_score(self, consent_report, data_requests_report) -> float:
        """
    ...
    def _generate_recommendations(self, consent_report, data_requests_report) -> List[str]:
        """
        ...
        if consent_report['summary']['expired_consents'] > 0:
            recommendations.append("Consider implementing automatic consent renewal reminders")

        if consent_report['summary']['revoked_consents'] > consent_report['summary']['total_consents'] * 0.1:
            recommendations.append("High consent revocation rate detected. Review consent process")

        # Data requests recommendations
        if data_requests_report['summary']['pending_requests'] > 0:
            recommendations.append("Address pending data subject requests to maintain GDPR compliance")

        if data_requests_report['processing_times']['average'] > 259200:  # 3 days in seconds
