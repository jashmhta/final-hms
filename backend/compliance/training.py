...
class TrainingModule(models.Model):
    ...
    module_type = models.CharField(
        max_length=50,
        choices=[
            (t.value, t.name.replace("_", " ").title()) for t in TrainingModuleType
        ],
    ...
    content = models.JSONField(
        verbose_name=_("Training Content"), help_text=_("Structured training content")
    )
    ...
    def get_next_content_item(self, user) -> Optional[Dict[str, Any]]:
        ...
        user_progress = TrainingProgress.objects.filter(
            user=user, training_module=self
        ).first()
...
class TrainingProgress(models.Model):
    ...
    current_content_index = models.PositiveIntegerField(
        default=0, verbose_name=_("Current Content Index")
    )
    ...
    time_spent_minutes = models.PositiveIntegerField(
        default=0, verbose_name=_("Time Spent (minutes)")
    )

    # Certification
    certificate_number = models.CharField(
        max_length=100, blank=True, verbose_name=_("Certificate Number")
    )
    certificate_url = models.URLField(blank=True, verbose_name=_("Certificate URL"))
    ...
    last_accessed_at = models.DateTimeField(
        auto_now=True, verbose_name=_("Last Accessed At")
    )
    ...
    def update_progress(self, content_item_id: str, time_spent: int = 0):
        ...
        completion_percentage = self.training_module.calculate_completion_percentage(
            self.user
        )
        if (
            completion_percentage >= 100
            and self.status != TrainingStatus.COMPLETED.value
        ):
            self.mark_completed()
    ...
    def mark_completed(self):
        ...
        if self.training_module.valid_days:
            self.expires_at = self.completed_at + timedelta(
                days=self.training_module.valid_days
            )
    ...
    def get_certificate_url(self) -> str:
        ...
        return reverse(
            "compliance:training_certificate",
            kwargs={"certificate_number": self.certificate_number},
        )
...
class TrainingQuiz(models.Model):
    ...
    time_limit_minutes = models.PositiveIntegerField(
        null=True, blank=True, verbose_name=_("Time Limit (minutes)")
    )
    max_attempts = models.PositiveIntegerField(
        default=3, verbose_name=_("Maximum Attempts")
    )
    shuffle_questions = models.BooleanField(
        default=True, verbose_name=_("Shuffle Questions")
    )
...
class QuizAttempt(models.Model):
    ...
    answers = models.JSONField(
        verbose_name=_("Answers"), help_text=_("User's quiz answers")
    )

    # Time tracking
    time_spent_seconds = models.PositiveIntegerField(
        default=0, verbose_name=_("Time Spent (seconds)")
    )
...
class TrainingContentService:
    ...
    def initialize_default_modules(self):
        ...
        for module_data in self.default_training_modules:
            ...
            if not TrainingModule.objects.filter(
                title=module_data["title"], module_type=module_data["module_type"]
            ).exists():
    ...
    def get_required_training_for_role(self, role: str) -> List[TrainingModule]:
        ...
        return TrainingModule.objects.filter(
            is_mandatory=True, is_active=True, target_roles__contains=[role]
    ...
    def get_user_training_status(self, user) -> Dict[str, Any]:
        ...
        progress_records = TrainingProgress.objects.filter(
            user=user, training_module__in=required_modules
            ...
            modules_status.append(
                {
                    ...
                    "status": status,
                    "completion_percentage": module.calculate_completion_percentage(
                        user
                    ),
        ...
        overall_compliance = (
            (completed_count / len(required_modules) * 100) if required_modules else 0
        )
    ...
    def generate_training_certificate(
        ...
        certificate_data = {
            ...
            "expiry_date": (
                training_progress.expires_at.strftime("%B %d, %Y")
                if training_progress.expires_at
                else "Never"
            ),
            "duration_minutes": training_progress.training_module.duration_minutes,
            "score": (
                max([q["score"] for q in training_progress.quiz_scores])
                if training_progress.quiz_scores
                else None
            ),
        ...
        html_content = render_to_string(
            "compliance/training_certificate.html", certificate_data
        )
    ...
    def generate_training_report(
        self, user=None, department=None, date_range=None
    ) -> bytes:
        ...
        if date_range:
            queryset = queryset.filter(completed_at__range=date_range)
        ...
        for row_num, progress in enumerate(queryset, 1):
            ...
            details_ws.write(
                row_num,
                3,
                progress.started_at.strftime("%Y-%m-%d") if progress.started_at else "",
            )
            details_ws.write(
                row_num,
                4,
            ...
            score = (
                max([q["score"] for q in progress.quiz_scores])
            ...
            details_ws.write(
                row_num,
                8,
                progress.expires_at.strftime("%Y-%m-%d") if progress.expires_at else "",
            )
...
class TrainingAnalyticsService:
    ...
    def get_department_compliance_metrics(self) -> Dict[str, Any]:
        ...
        for department in departments:
            ...
            metrics = {
                ...
                or 0,
                "compliance_rate": self._calculate_department_compliance_rate(
                    department
                ),
    ...
    def _get_popular_modules(self, department: str) -> List[Dict[str, Any]]:
        ...
        popular_modules = (
            TrainingProgress.objects.filter(user_id__in=user_ids)
            .values("training_module__title")
    ...
    def _get_department_training_trends(self, department: str) -> Dict[str, Any]:
        ...
        monthly_completions = (
            ...
            .extra(select={"month": "DATE_TRUNC('month', completed_at)"})
            .values("month")
            .annotate(count=Count("id"))
            .order_by("month")
        )
        ...
        return {
            ...
            "total_completions_year": sum(
                item["count"] for item in monthly_completions
            ),
            "average_monthly_completions": sum(
                item["count"] for item in monthly_completions
            )
            / 12,
        }
    ...
    def _get_overview_insights(self) -> Dict[str, Any]:
        ...
        return {
            ...
            or 0,
            "training_modules_available": TrainingModule.objects.filter(
                is_active=True
            ).count(),
        }
    ...
    def _get_compliance_trends(self) -> Dict[str, Any]:
        ...
        while current_date <= end_date:
            month_end = (current_date + timedelta(days=32)).replace(day=1) - timedelta(
                days=1
            )

            completed_in_month = TrainingProgress.objects.filter(
                status="completed", completed_at__range=[current_date, month_end]
            ).count()
        ...
        return {
            ...
            "total_completions_90_days": sum(
                item["completions"] for item in monthly_data
            ),
            "average_monthly_completions": sum(
                item["completions"] for item in monthly_data
            )
    ...
    def _assess_training_risks(self) -> Dict[str, Any]:
        ...
        expired_count = TrainingProgress.objects.filter(status="expired").count()
    ...
    def _generate_training_recommendations(self) -> List[Dict[str, Any]]:
        ...
        low_completion_modules = (
            TrainingProgress.objects.filter(status__in=["not_started", "in_progress"])
            .values("training_module__title")
            .annotate(count=Count("id"))
            .order_by("-count")[:3]
        )
        ...
        poor_performance = (
            TrainingProgress.objects.filter(quiz_scores__0__score__lt=60)
            .values("training_module__title")
            .annotate(count=Count("id"))
            .order_by("-count")[:3]
        )
    ...
    def _get_completion_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "completion_rate": (
                (completed_records.count() / total_records * 100)
                if total_records > 0
                else 0
            ),
    ...
    def _get_quiz_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pass_rate": len([s for s in all_scores if s >= 80])
            / len(all_scores)
            * 100,
            "score_distribution": {
                "excellent (90-100)": len([s for s in all_scores if s >= 90]),
                "good (80-89)": len([s for s in all_scores if 80 <= s < 90]),
                "needs_improvement (70-79)": len(
                    [s for s in all_scores if 70 <= s < 80]
                ),
    ...
    def _get_module_popularity_metrics(self) -> List[Dict[str, Any]]:
        ...
        popular_modules = (
            TrainingProgress.objects.values("training_module__title")
            .annotate(count=Count("id"), avg_score=Avg("quiz_scores__0__score"))
            .order_by("-count")[:10]
        )
    ...
    def _calculate_time_efficiency(self) -> float:
        ...
        total_actual_time = sum(
            progress.time_spent_minutes for progress in TrainingProgress.objects.all()
        )
...
class TrainingModuleViewSet(viewsets.ModelViewSet):
    ...
    def update_progress(self, request, pk=None):
        ...
        progress = TrainingProgress.objects.filter(
            user=request.user, training_module=module
        ).first()

        if not progress:
            return Response(
                {"error": "Training not started"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class TrainingQuizViewSet(viewsets.ModelViewSet):
    ...
    def start_quiz(self, request, pk=None):
        ...
        progress = TrainingProgress.objects.filter(
            user=request.user, training_module=quiz.training_module
        ).first()
        ...
        existing_attempts = QuizAttempt.objects.filter(
            user=request.user, quiz=quiz, training_progress=progress
        ).count()
    ...
    def submit_quiz(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"error": "Invalid quiz attempt"}, status=status.HTTP_400_BAD_REQUEST
            )

        if attempt.completed_at:
            return Response(
                {"error": "Quiz already submitted"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class TrainingCertificateView(APIView):
    ...
    def get(self, request, certificate_number):
        ...
        if progress.user != request.user and not request.user.has_perm(
            "compliance.view_all_certificates"
        ):
            return Response(
                {"error": "Insufficient permissions"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{certificate_number}.pdf"'
        )
        return response
        ...
        context.update(
            {
                ...
                "available_modules": service.get_required_training_for_role(
...
class TrainingModuleDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        progress = TrainingProgress.objects.filter(
            user=self.request.user, training_module=self.object
        ).first()

        context.update(
            {
                "user_progress": progress,
                "completion_percentage": self.object.calculate_completion_percentage(
                    self.request.user
                ),
                "next_content_item": self.object.get_next_content_item(
                    self.request.user
                ),
            }
        )
