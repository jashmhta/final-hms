...
class HIPAASafeguard(models.Model):
    ...
    safeguard_type = models.CharField(
        max_length=20,
        choices=[
            (t.value, t.name.replace("_", " ").title()) for t in HIPAASafeguardType
        ],
    )
    specification = models.CharField(
        max_length=30,
        choices=[
            (s.value, s.name.replace("_", " ").title()) for s in HIPAASpecification
        ],
    )
    ...
    def check_compliance(self) -> Tuple[bool, List[str]]:
        ...
        if self.implementation_status not in ["IMPLEMENTED", "NOT_APPLICABLE"]:
            issues.append(
                f"Safeguard not fully implemented: {self.implementation_status}"
            )
            is_compliant = False
...
class PHIAccessLog(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="phi_access_logs"
    )
    ...
    retention_until = models.DateTimeField(
        default=lambda: timezone.now() + timedelta(days=365 * 7)
    )  # 7 years
...
class HIPAATrainingRecord(models.Model):
    ...
    # Records retention
    retention_until = models.DateTimeField(
        default=lambda: timezone.now() + timedelta(days=365 * 6)
    )  # 6 years

    # Metadata
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_trainings"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    is_active = models.BooleanField(default=True)
...
class HIPAATrainingAttendance(models.Model):
    ...
    training = models.ForeignKey(
        HIPAATrainingRecord, on_delete=models.CASCADE, related_name="attendance_records"
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="hipaa_training_records"
    )
...
class BusinessAssociateAgreement(models.Model):
    ...
    internal_contact = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="managed_baas"
    )
...
class HIPAAComplianceManager:
    ...
    def perform_risk_assessment(self, scope: Dict = None) -> HIPAARiskAssessment:
        ...
        unauthorized_access = PHIAccessLog.objects.filter(
            was_authorized=False, timestamp__gte=timezone.now() - timedelta(days=365)
        ).count()
    ...
    def generate_compliance_report(self) -> Dict:
        ...
        safeguard_stats = {
            ...
            "implemented": safeguards.filter(
                implementation_status="IMPLEMENTED"
            ).count(),
            "partially_implemented": safeguards.filter(
                implementation_status="PARTIALLY_IMPLEMENTED"
            ).count(),
            "not_implemented": safeguards.filter(
        ...
        phi_stats = {
            ...
            "suspicious_access": PHIAccessLog.objects.filter(
                is_suspicious=True
            ).count(),
        ...
        training_stats = {
            ...
            "trained_employees": HIPAATrainingAttendance.objects.filter(
                passed_assessment=True, certificate_expiry__gte=timezone.now().date()
            )
            .values("user")
            .distinct()
            .count(),
        ...
        training_stats["compliance_rate"] = (
            (
                training_stats["trained_employees"]
                / training_stats["total_employees"]
                * 100
            )
            if training_stats["total_employees"] > 0
            else 0
        )
        ...
        baa_stats = {
            ...
            "active_agreements": BusinessAssociateAgreement.objects.filter(
                status="ACTIVE"
            ).count(),
        ...
        implemented_safeguards = (
            safeguard_stats["implemented"]
            + safeguard_stats["partially_implemented"] * 0.5
        )
        safeguard_compliance = (
            (implemented_safeguards / safeguard_stats["total"] * 100)
            if safeguard_stats["total"] > 0
            else 0
        )
        ...
        baa_compliance = (
            ...
            if baa_stats["total_agreements"] > 0
            else 100
        )

        report["overall_compliance_score"] = round(
            (
                safeguard_compliance * 0.5
                + training_compliance * 0.3
                + baa_compliance * 0.2
            ),
            2,
        )
    ...
    def check_breach_notification_requirements(self, incident_data: Dict) -> Dict:
        ...
        return {
            ...
            "hhs_notification_required": requires_notification
            and affected_individuals > 500,
        }
...
def create_default_hipaa_safeguards():
    ...
    for safeguard_data in safeguards_data:
        HIPAASafeguard.objects.get_or_create(
            name=safeguard_data["name"], defaults=safeguard_data
        )
