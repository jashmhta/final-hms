...
class MicroservicesTestSuite:
    ...
    def test_port_connectivity(self, host: str, port: int, timeout: float = 5.0) -> bool:
        try:
    ...
    def test_service_health(self, service_config: ServiceConfig) -> ServiceTestResult:
        ...
        try:
            ...
            if is_accessible:
                ...
                self.logger.info(f"✅ {service_name} is accessible ({response_time:.3f}s)")
                for endpoint in service_config.expected_endpoints or []:
                    result.endpoints_tested += 1
                    try:
                        url = f"http://{host}:{port}{endpoint}"
                        curl_cmd = ['curl', '-s', '-o', '/dev/null', '-w', '%{http_code}', url]
                        process = subprocess.run(curl_cmd, capture_output=True, timeout=10)
                        http_code = process.stdout.decode().strip()
                        if http_code == '200':
                            result.endpoints_passed += 1
                            self.logger.info(f"✅ {service_name} {endpoint} endpoint accessible (HTTP 200)")
                        else:
                            result.errors.append(f"Endpoint {endpoint} returned HTTP {http_code}")
                            self.logger.warning(f"⚠️ {service_name} {endpoint} returned HTTP {http_code}")
    ...
    def generate_test_report(self) -> Dict[str, Any]:
        ...
        healthy_services = sum(1 for r in self.test_results.values() if r.status == "REACHABLE")
        critical_services = [name for name, config in self.service_configs.items()
                           if config.is_critical and self.test_results.get(name) and self.test_results[name].status != "REACHABLE"]
    ...
    def print_test_summary(self):
        ...
        for status, count in sorted(status_counts.items()):
            emoji = {"REACHABLE": "✅", "UNREACHABLE": "🔌", "ERROR": "💥"}.get(status, "❓")
        ...
        for name, result in sorted(self.test_results.items()):
            ...
            status_emoji = {"REACHABLE": "✅", "UNREACHABLE": "🔌", "ERROR": "💥"}.get(result.status, "❓")
            print(f"   {critical_marker} {status_emoji} {name}: {result.status} ({result.response_time:.3f}s)")
            if result.errors:
