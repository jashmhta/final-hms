...
PROCESSING_LATENCY = prometheus_client.Histogram(
    "hms_data_processing_latency_seconds", "Data processing latency", ["data_type"]
)

DATA_QUALITY_SCORE = prometheus_client.Gauge(
    "hms_data_quality_score", "Data quality score (0-100)", ["data_type"]
)
...
class RealtimeDataIngestor:
    ...
    async def ingest_patient_data(self, data_stream: AsyncGenerator[Dict, None]):
        ...
        async for data in data_stream:
            ...
            try:
                ...
                DATA_QUALITY_SCORE.labels(data_type=validated_data["type"]).set(
                    quality_score
                )

                if quality_score < 70:
                    logger.warning(
                        f"Low data quality score: {quality_score} for {validated_data['type']}"
                    )
                ...
                logger.info(
                    f"Successfully ingested {enriched_data['type']} data for patient {enriched_data['patient_id']}"
                )
        ...
        try:
            ...
            if result:
                ...
                self.redis_client.setex(
                    cache_key, 300, json.dumps(dict(result), default=str)
                )
                return dict(result)
    ...
    async def store_in_cache(self, data: Dict):
        ...
        self.redis_client.zadd(
            timeline_key, {json.dumps(timeline_entry): datetime.utcnow().timestamp()}
        )
        ...
        self.redis_client.setex(
            latest_key, 3600, json.dumps(data, default=str)  # 1 hour TTL
        )
    ...
    async def send_to_kafka(self, data: Dict):
        ...
        try:
            self.kafka_producer.send(
                topic, value=data, key=data["patient_id"].encode("utf-8")
            )
    ...
    async def send_to_quality_issue_queue(self, data: Dict):
        ...
        self.kafka_producer.send("hms_data_quality_issues", value=quality_issue)
    ...
    async def handle_ingestion_error(self, data: Dict, error: Exception):
        ...
        self.kafka_producer.send("hms_ingestion_errors", value=error_record)

    async def create_stream_processing_pipeline(self):
        """Create Apache Beam pipeline for batch processing"""
        options = PipelineOptions(
            ["--runner=DirectRunner", "--job_name=hms-data-processing"]
        )
        ...
        with beam.Pipeline(options=options) as p:
            ...
            (
                ...
                | "DecodeMessages"
                >> beam.Map(lambda msg: json.loads(msg.value.decode("utf-8")))
    ...
    async def send_alert(self, alert: Dict):
        """Send clinical alert"""
        self.kafka_producer.send("hms_clinical_alerts", value=alert)
