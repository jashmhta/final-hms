...
ENTITY_EXTRACTION_COUNT = prometheus_client.Counter(
    'hms_entity_extraction_total',
    'Total entities extracted',
    ['entity_type']
)

NLP_PROCESSING_LATENCY = prometheus_client.Histogram(
    'hms_nlp_processing_latency_seconds',
...
class ClinicalNLPProcessor:
    ...
    def __init__(self):
        ...
        self.classifier = AutoModelForSequenceClassification.from_pretrained("emilyalsentzer/Bio_ClinicalBERT")
        ...
        self.sentiment_analyzer = pipeline("sentiment-analysis", model="siebert/sentiment-roberta-large-english")
    ...
    def add_medical_patterns(self):
        ...
        patterns = [
            {"label": "MEDICATION", "pattern": [{"LOWER": {"REGEX": r"^[a-z]+(ol|in|one|ium|ide|pine)$"}}]},
            {"label": "DOSAGE", "pattern": [{"SHAPE": "dd"}, {"LOWER": "mg"}, {"LOWER": "daily"}]},
            {"label": "CONDITION", "pattern": [{"LOWER": {"REGEX": r"^(diabetes|hypertension|asthma|copd|heart failure|pneumonia)$"}}]}
        ]
    ...
    def _load_medications_database(self):
        ...
        return {
            ...
            'amlodipine': {'generic_name': 'amlodipine', 'class': 'calcium channel blocker'}
        }
    ...
    def _load_conditions_database(self):
        ...
        return {
            ...
            'copd': {'full_name': 'chronic obstructive pulmonary disease', 'icd10': 'J44'},
    ...
    def process_clinical_note(self, note: ClinicalNote) -> NLPExtractionResult:
        ...
        # Update metrics
        NLP_PROCESSED_NOTES.labels(note_type=note.note_type, extraction_type='full').inc()
        NLP_PROCESSING_LATENCY.labels(note_type=note.note_type).observe(time.time() - start_time)
    ...
    def _extract_procedures(self, text: str) -> List[Dict]:
        ...
        procedure_keywords = [
            'surgery', 'operation', 'procedure', 'biopsy', 'endoscopy',
            'colonoscopy', 'catheterization', 'intubation', 'ventilation'
        ]
    ...
    def _extract_family_history(self, text: str) -> List[Dict]:
        ...
        matches = re.finditer(self.medical_patterns['family_history'], text, re.IGNORECASE)
        for match in matches:
    ...
    def _extract_plan(self, text: str) -> List[str]:
        ...
        if 'plan' in sections:
            ...
            actions = re.findall(r'(will|plan to|recommend|suggest|order)\s+([^\.;]+)', plan_text, re.IGNORECASE)
    ...
    def _assess_note_quality(self, text: str) -> Dict:
        ...
        quality_metrics['structure'] = len([s for s in required_sections if s in sections]) / len(required_sections) * 100
    ...
    def _split_into_sections(self, text: str) -> Dict[str, str]:
        ...
        sections = {}
        section_patterns = [
            (r'subjective[:\-]?\s*(.*?)(?=objective[:\-]|assessment[:\-]|plan[:\-]|$)', 'subjective'),
            (r'objective[:\-]?\s*(.*?)(?=subjective[:\-]|assessment[:\-]|plan[:\-]|$)', 'objective'),
            (r'assessment[:\-]?\s*(.*?)(?=subjective[:\-]|objective[:\-]|plan[:\-]|$)', 'assessment'),
            (r'plan[:\-]?\s*(.*?)(?=subjective[:\-]|objective[:\-]|assessment[:\-]|$)', 'plan')
        ]
    ...
    def _extract_smoking_status(self, text: str) -> str:
        ...
        for pattern, status in smoking_patterns:
            ...
            if match:
                return status if status != 'pack_years' else f"{match.group(1)} pack-years"
        return 'unknown'
    ...
    def _extract_substance_use(self, text: str) -> str:
        ...
        if re.search(r'no illicit drugs|no substance use', text, re.IGNORECASE):
            return 'none'
        elif re.search(r'heroin|cocaine|marijuana|methamphetamine', text, re.IGNORECASE):
            return 'positive'
        return 'unknown'
    ...
    def _extract_living_situation(self, text: str) -> str:
        ...
        match = re.search(r'(?:lives? with|living situation)[:\-]?\s*([^\.;]+)', text, re.IGNORECASE)
        return match.group(1).strip() if match else 'unknown'
