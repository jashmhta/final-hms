    ...
    def start_consuming(self, queue: str):
        ...
        def callback(ch, method, properties, body):
            try:
                ...
                if handler:
                    logger.info(
                        f"Processing message from {method.routing_key}",
                        correlation_id=correlation_id,
                    )
                ...
                logger.error(
                    f"Failed to process message: {e}", correlation_id=correlation_id
                )
...
class KafkaCorrelationProducer:
    ...
    def send(
        ...
        if tracer:
            ...
            if current_span:
                message_headers.update(
                    {
                        b"x-trace-id": str(current_span.context.trace_id).encode(
                            "utf-8"
                        ),
        ...
        future = self.producer.send(
            topic, value=value, key=key, headers=message_headers
        )
...
class RedisCorrelationPublisher:
    ...
    def publish(
        ...
        logger.info(
            f"Published to Redis channel {channel}", correlation_id=correlation_id
        )
...
class CorrelationTask(Task):
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            logger.info(f"Completed task {self.name}", correlation_id=correlation_id)
            return result

        except Exception as e:
            logger.error(
                f"Task {self.name} failed", error=str(e), correlation_id=correlation_id
            )
            raise
    ...
    def wrapper(message, *args, **kwargs):
        ...
        if isinstance(message, dict):
            ...
            correlation_id = (
                ...
                or message.get("_metadata", {}).get("correlation_id")
                or message.get("headers", {}).get("x-correlation-id")
            )
...
def message_processing_context(message_data: Dict[str, Any]):
    ...
    if isinstance(message_data, dict):
        correlation_id = (
            ...
            or message_data.get("_metadata", {}).get("correlation_id")
            or message_data.get("headers", {}).get("x-correlation-id")
        )
