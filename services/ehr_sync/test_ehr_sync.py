...
class TestEHRSynchronizationService:
    ...
    class TestInitialization:
        ...
        async def test_initialize_with_encryption_key(self):
            ...
            with patch.dict('os.environ', {'EHR_ENCRYPTION_KEY': 'test-key-for-encryption'}):
                service = EHRSynchronizationService()
                with patch.object(service, 'initialize', return_value=None):
                    await service.initialize()
    ...
    class TestEHRSystemRegistration:
        ...
        @pytest.mark.asyncio
        async def test_register_ehr_system_success(self, ehr_service, sample_ehr_config):
        ...
        async def test_register_ehr_system_missing_fields(self, ehr_service):
            ...
            incomplete_config = {"name": "Test System"}  # Missing system_type and base_url
            with pytest.raises(ValueError, match="Required field 'system_type' is missing"):
                await ehr_service.register_ehr_system(incomplete_config)

        @pytest.mark.asyncio
        async def test_register_ehr_system_connectivity_failure(self, ehr_service, sample_ehr_config):
            """Test EHR system registration with connectivity failure"""
            with patch.object(ehr_service, '_test_ehr_connectivity', return_value=False):
    ...
    class TestDataTransformation:
        ...
        @pytest.mark.asyncio
        async def test_transform_data_no_mappings(self, ehr_service, sample_patient_data):
            """Test data transformation with no mappings"""
            ehr_system = Mock()
            ehr_system.id = "test_system_id"

            transformed_data = await ehr_service._transform_data(sample_patient_data, ehr_system)
    ...
    class TestFHIRConversion:
        ...
        @pytest.mark.asyncio
        async def test_convert_to_fhir_patient_basic(self, ehr_service, sample_patient_data):
            """Test basic FHIR patient conversion"""
            fhir_patient = await ehr_service._convert_to_fhir_patient(sample_patient_data)
        ...
        @pytest.mark.asyncio
        async def test_convert_to_fhir_patient_with_identifiers(self, ehr_service, sample_patient_data):
            """Test FHIR patient conversion with identifiers"""
            fhir_patient = await ehr_service._convert_to_fhir_patient(sample_patient_data)

            assert len(fhir_patient["identifier"]) == 2
            mrn_identifier = next((id for id in fhir_patient["identifier"] if id["system"] == "http://hospital.com/mrn"), None)
            assert mrn_identifier is not None
            assert mrn_identifier["value"] == "MRN001"

        @pytest.mark.asyncio
        async def test_convert_to_fhir_patient_with_contact(self, ehr_service, sample_patient_data):
            """Test FHIR patient conversion with contact information"""
            fhir_patient = await ehr_service._convert_to_fhir_patient(sample_patient_data)

            assert len(fhir_patient["telecom"]) == 2
            phone_telecom = next((tel for tel in fhir_patient["telecom"] if tel["system"] == "phone"), None)
            assert phone_telecom is not None
            assert phone_telecom["value"] == "+1-555-123-4567"

        @pytest.mark.asyncio
        async def test_convert_to_fhir_patient_with_address(self, ehr_service, sample_patient_data):
    ...
    class TestHL7Conversion:
        ...
        @pytest.mark.asyncio
        async def test_convert_to_hl7_patient_create(self, ehr_service, sample_patient_data):
            """Test HL7 patient conversion for create operation"""
            hl7_message = await ehr_service._convert_to_hl7_patient(sample_patient_data, "CREATE")
        ...
        @pytest.mark.asyncio
        async def test_convert_to_hl7_patient_unsupported_type(self, ehr_service, sample_patient_data):
            """Test HL7 patient conversion with unsupported type"""
            hl7_message = await ehr_service._convert_to_hl7_patient(sample_patient_data, "UNSUPPORTED")
            assert hl7_message == ""
    ...
    class TestSynchronization:
        ...
        @pytest.mark.asyncio
        async def test_sync_patient_fhir_success(self, ehr_service, sample_patient_data):
            ...
            with patch.object(ehr_service, '_get_ehr_system', return_value=ehr_system), \
                 patch.object(ehr_service, '_transform_data', return_value=sample_patient_data), \
                 patch.object(ehr_service, '_get_ehr_headers', return_value={"Authorization": "Bearer test"}), \
                ...
                result = await ehr_service._sync_patient_fhir(request, ehr_system, sample_patient_data)
        ...
        async def test_sync_patient_hl7_success(self, ehr_service, sample_patient_data):
            ...
            ehr_service.hl7_processor.parse_ack = Mock(return_value={"acknowledgement_code": "AA"})
            ...
            with patch.object(ehr_service, '_get_ehr_system', return_value=ehr_system), \
                ...
                result = await ehr_service._sync_patient_hl7(request, ehr_system, sample_patient_data)

                assert result.status == SyncStatus.COMPLETED
                assert result.entity_id == "PAT001"

        @pytest.mark.asyncio
        async def test_sync_patient_unsupported_format(self, ehr_service, sample_patient_data):
            ...
            with pytest.raises(ValueError, match="Unsupported data format: UNSUPPORTED"):
                await ehr_service._sync_patient_fhir(request, ehr_system, sample_patient_data)
    ...
    class TestConnectivity:
        ...
        async def test_test_ehr_connectivity_success(self, ehr_service):
            ...
            with patch.object(ehr_service, '_get_ehr_headers', return_value={"Authorization": "Bearer test"}), \
...
class TestIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_patient_sync_workflow(self, ehr_service, sample_patient_data, sample_ehr_config):
        """Test complete patient synchronization workflow"""
        with patch.object(ehr_service, 'register_ehr_system', return_value="test_system_id"), \
             patch.object(ehr_service, '_get_ehr_system') as mock_get_system, \
             patch.object(ehr_service, '_transform_data', return_value=sample_patient_data), \
             patch.object(ehr_service, '_get_ehr_headers', return_value={"Authorization": "Bearer test"}), \
             patch('aiohttp.ClientSession.post') as mock_post:
...
class TestPerformance:
    ...
    @pytest.mark.asyncio
    async def test_bulk_patient_sync_performance(self, ehr_service, sample_patient_data):
        ...
        with patch.object(ehr_service, '_get_ehr_system') as mock_get_system, \
             patch.object(ehr_service, '_transform_data', side_effect=lambda x, y: x), \
             patch.object(ehr_service, '_get_ehr_headers', return_value={"Authorization": "Bearer test"}), \
             patch('aiohttp.ClientSession.post') as mock_post:
            ...
            successful_syncs = sum(1 for r in results if isinstance(r, type(EHRSyncResponse)) and r.status == SyncStatus.COMPLETED)

            assert successful_syncs == 100
            assert total_time < 30  # Should complete within 30 seconds
            print(f"Bulk sync completed in {total_time:.2f} seconds ({successful_syncs/total_time:.2f} syncs/sec)")
