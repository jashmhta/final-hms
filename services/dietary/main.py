...
# Patient Diet Requirements endpoints
@app.post("/patient-requirements/", response_model=schemas.PatientDietRequirement)
def create_patient_diet_requirement(requirement: schemas.PatientDietRequirementCreate, db: Session = Depends(get_db)):
    return crud.create_patient_diet_requirement(db, requirement)

@app.get("/patient-requirements/{patient_id}", response_model=schemas.PatientDietRequirement)
def read_patient_diet_requirement(patient_id: str, db: Session = Depends(get_db)):
    ...
    if requirement is None:
        raise HTTPException(status_code=404, detail="Patient diet requirement not found")
    return requirement

@app.put("/patient-requirements/{patient_id}", response_model=schemas.PatientDietRequirement)
def update_patient_diet_requirement(patient_id: str, requirement: schemas.PatientDietRequirementCreate, db: Session = Depends(get_db)):
...
# Nutrition Calculations endpoints
@app.post("/nutrition-calculations/", response_model=schemas.NutritionCalculation)
def create_nutrition_calculation(calculation: schemas.NutritionCalculationCreate, db: Session = Depends(get_db)):
    return crud.create_nutrition_calculation(db, calculation)

@app.get("/nutrition-calculations/{patient_id}", response_model=schemas.NutritionCalculation)
...
@app.put("/nutrition-calculations/{patient_id}", response_model=schemas.NutritionCalculation)
def update_nutrition_calculation(patient_id: str, calculation: schemas.NutritionCalculationCreate, db: Session = Depends(get_db)):
