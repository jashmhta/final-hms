...
def get_patient_diet_requirement(db: Session, patient_id: str):
    return (
        db.query(models.PatientDietRequirement)
        .filter(models.PatientDietRequirement.patient_id == patient_id)
        .first()
    )


def create_patient_diet_requirement(
    db: Session, requirement: schemas.PatientDietRequirementCreate
):
...
def update_patient_diet_requirement(
    db: Session, patient_id: str, requirement: schemas.PatientDietRequirementCreate
):
    db_requirement = (
        db.query(models.PatientDietRequirement)
        .filter(models.PatientDietRequirement.patient_id == patient_id)
...
def get_patient_meal_plans(db: Session, patient_id: str):
    return (
        db.query(models.MealPlan).filter(models.MealPlan.patient_id == patient_id).all()
    )
...
def get_nutrition_calculation(db: Session, patient_id: str):
    return (
        db.query(models.NutritionCalculation)
        .filter(models.NutritionCalculation.patient_id == patient_id)
        .first()
    )


def create_nutrition_calculation(
    db: Session, calculation: schemas.NutritionCalculationCreate
):
...
def update_nutrition_calculation(
    db: Session, patient_id: str, calculation: schemas.NutritionCalculationCreate
):
    db_calc = (
        db.query(models.NutritionCalculation)
        .filter(models.NutritionCalculation.patient_id == patient_id)
...
def get_dietary_statistics(db: Session):
    ...
    active_meal_plans = (
        db.query(models.MealPlan).filter(models.MealPlan.is_active == True).count()
    )
    meals_served_today = (
...
def get_dietary_dashboard(db: Session):
    todays_meals = (
        db.query(models.Meal)
        .filter(models.Meal.meal_date == datetime.utcnow().date())
        .count()
    )
    pending_meals = (
        db.query(models.Meal)
    ...
    special_diets = (
        db.query(models.PatientDietRequirement)
        .filter(models.PatientDietRequirement.diet_type != schemas.DietType.REGULAR)
        .count()
    )
    nutrition_calculations_today = (
        db.query(models.NutritionCalculation)
        .filter(
