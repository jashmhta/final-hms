...
def get_patient_appointments(
    db: Session, patient_id: int, status: Optional[str] = None
):
    query = db.query(models.PatientAppointment).filter(
...
def create_patient_appointment(
    db: Session, appointment: schemas.PatientAppointmentCreate
):
...
def update_appointment_status(db: Session, appointment_id: str, status: str):
    db_appointment = (
        db.query(models.PatientAppointment)
        .filter(models.PatientAppointment.appointment_id == appointment_id)
        .first()
    )
    if db_appointment:
...
def get_patient_medical_records(
    db: Session, patient_id: int, record_type: Optional[str] = None
):
    query = db.query(models.PatientMedicalRecord).filter(
...
def create_patient_medical_record(
    db: Session, record: schemas.PatientMedicalRecordCreate
):
...
def get_patient_feedback(
    db: Session, patient_id: int, service_type: Optional[str] = None
):
    query = db.query(models.PatientFeedback).filter(
        models.PatientFeedback.patient_id == patient_id
    )
    if service_type:
...
def get_patient_notifications(
    db: Session, patient_id: int, is_read: Optional[bool] = None
):
    query = db.query(models.PatientNotification).filter(
...
def create_patient_notification(
    db: Session, notification: schemas.PatientNotificationCreate
):
...
def mark_notification_read(db: Session, notification_id: str):
    db_notification = (
        db.query(models.PatientNotification)
        .filter(models.PatientNotification.notification_id == notification_id)
        .first()
    )
    if db_notification:
...
def get_patient_dashboard(db: Session, patient_id: int):
    ...
    pending_results = (
        ...
        .filter(
            ...
            models.PatientMedicalRecord.created_at
