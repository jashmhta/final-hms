...
def update_patient(
    patient_id: int, patient_update: dict, db: Session = Depends(get_db)
):
...
def create_patient_appointment(
    appointment: schemas.PatientAppointmentCreate, db: Session = Depends(get_db)
):
...
@app.get(
    ...
    response_model=List[schemas.PatientAppointment],
)
def read_patient_appointments(
    patient_id: int, status: Optional[str] = None, db: Session = Depends(get_db)
):
...
def create_patient_medical_record(
    record: schemas.PatientMedicalRecordCreate, db: Session = Depends(get_db)
):
...
@app.get(
    ...
    response_model=List[schemas.PatientMedicalRecord],
)
def read_patient_medical_records(
    patient_id: int, record_type: Optional[str] = None, db: Session = Depends(get_db)
):
...
def create_patient_feedback(
    feedback: schemas.PatientFeedbackCreate, db: Session = Depends(get_db)
):
...
def read_patient_feedback(
    patient_id: int, service_type: Optional[str] = None, db: Session = Depends(get_db)
):
...
def create_patient_notification(
    notification: schemas.PatientNotificationCreate, db: Session = Depends(get_db)
):
    return crud.create_patient_notification(db, notification)


@app.get(
    "/notifications/patient/{patient_id}",
    response_model=List[schemas.PatientNotification],
)
def read_patient_notifications(
    patient_id: int, is_read: Optional[bool] = None, db: Session = Depends(get_db)
):
