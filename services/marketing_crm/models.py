...
class Lead(Base):
    ...
    source = Column(String)  # website, referral, event, etc.
    status = Column(Enum(LeadStatus), default=LeadStatus.NEW)
    priority = Column(String)  # high, medium, low
    assigned_to = Column(String, nullable=True)  # sales representative
...
class Customer(Base):
    ...
    customer_type = Column(String)  # individual, corporate, government
    acquisition_date = Column(DateTime)
    last_contact_date = Column(DateTime, nullable=True)
    total_value = Column(Float, default=0)
    status = Column(String)  # active, inactive, churned
    notes = Column(Text, nullable=True)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Opportunity(Base):
    ...
    description = Column(Text, nullable=True)
    stage = Column(String)  # prospecting, qualification, proposal, negotiation, closed
...
class MarketingActivity(Base):
    ...
    activity_type = Column(String)  # webinar, whitepaper, case_study, etc.
    description = Column(Text, nullable=True)
    target_audience = Column(JSON, nullable=True)
    start_date = Column(DateTime)
    end_date = Column(DateTime, nullable=True)
    ...
    metrics = Column(JSON, nullable=True)  # views, downloads, registrations, etc.
    created_by = Column(String)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)