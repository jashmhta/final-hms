...
def update_lead_status(lead_id: int, status: schemas.LeadStatus, db: Session = Depends(get_db)):
...
def read_campaigns_by_status(status: schemas.CampaignStatus, db: Session = Depends(get_db)):
    return crud.get_campaigns_by_status(db, status)

@app.get("/campaigns/type/{campaign_type}", response_model=List[schemas.Campaign])
def read_campaigns_by_type(campaign_type: schemas.CampaignType, db: Session = Depends(get_db)):
    return crud.get_campaigns_by_type(db, campaign_type)

@app.patch("/campaigns/{campaign_id}/status")
def update_campaign_status(campaign_id: int, status: schemas.CampaignStatus, db: Session = Depends(get_db)):
...
def update_customer_value(customer_id: int, additional_value: float, db: Session = Depends(get_db)):
...
@app.get("/opportunities/customer/{customer_id}", response_model=List[schemas.Opportunity])
...
@app.get("/opportunities/assigned/{assigned_to}", response_model=List[schemas.Opportunity])
...
def update_opportunity_stage(opportunity_id: int, stage: str, probability: float, db: Session = Depends(get_db)):
...
def create_marketing_activity(activity: schemas.MarketingActivityCreate, db: Session = Depends(get_db)):
    return crud.create_marketing_activity(db, activity)

@app.get("/activities/", response_model=List[schemas.MarketingActivity])
def read_marketing_activities(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
@app.get("/activities/type/{activity_type}", response_model=List[schemas.MarketingActivity])
...
def update_activity_metrics(activity_id: int, metrics: dict, db: Session = Depends(get_db)):
