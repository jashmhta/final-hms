...
def get_campaigns_by_type(db: Session, campaign_type: schemas.CampaignType):
    return (
        ...
        .filter(models.Campaign.campaign_type == campaign_type)
        .all()
    )
...
def update_campaign_status(
    db: Session, campaign_id: int, status: schemas.CampaignStatus
):
    db_campaign = (
...
def get_interactions_by_lead(db: Session, lead_id: int):
    return (
        db.query(models.Interaction).filter(models.Interaction.lead_id == lead_id).all()
    )
...
def get_customer_by_customer_id(db: Session, customer_id_str: str):
    return (
        ...
        .filter(models.Customer.customer_id == customer_id_str)
        .first()
    )
...
def get_customers_by_type(db: Session, customer_type: str):
    return (
        db.query(models.Customer)
        .filter(models.Customer.customer_type == customer_type)
        .all()
    )
...
def get_opportunities_by_customer(db: Session, customer_id: int):
    return (
        db.query(models.Opportunity)
        .filter(models.Opportunity.customer_id == customer_id)
        .all()
    )
...
def get_opportunities_by_assigned_to(db: Session, assigned_to: str):
    return (
        db.query(models.Opportunity)
        .filter(models.Opportunity.assigned_to == assigned_to)
        .all()
    )
...
def update_opportunity_stage(
    db: Session, opportunity_id: int, stage: str, probability: float
):
    db_opportunity = (
        db.query(models.Opportunity)
        .filter(models.Opportunity.id == opportunity_id)
...
def get_marketing_activity(db: Session, activity_id: int):
    return (
        db.query(models.MarketingActivity)
        .filter(models.MarketingActivity.id == activity_id)
        .first()
    )
...
def get_activities_by_status(db: Session, status: str):
    return (
        db.query(models.MarketingActivity)
        .filter(models.MarketingActivity.status == status)
        .all()
    )
...
def get_activities_by_type(db: Session, activity_type: str):
    return (
        db.query(models.MarketingActivity)
        .filter(models.MarketingActivity.activity_type == activity_type)
        .all()
    )
...
def get_crm_statistics(db: Session):
    ...
    new_leads = (
        ...
        .filter(models.Lead.status == models.LeadStatus.NEW)
        .count()
    )
    qualified_leads = (
        db.query(models.Lead)
        .filter(models.Lead.status == models.LeadStatus.QUALIFIED)
        .count()
    )
    total_customers = db.query(models.Customer).count()
    active_customers = (
        db.query(models.Customer).filter(models.Customer.status == "active").count()
    )
    total_opportunities = db.query(models.Opportunity).count()
    open_opportunities = (
        db.query(models.Opportunity)
        .filter(
            models.Opportunity.stage.in_(
    ...
    converted_leads = (
        db.query(models.Lead)
        .filter(models.Lead.status == models.LeadStatus.CLOSED_WON)
        .count()
    )
    conversion_rate = (converted_leads / total_leads * 100) if total_leads > 0 else 0.0
    opportunities = db.query(models.Opportunity).all()
    average_deal_size = (
        sum(o.expected_value for o in opportunities) / len(opportunities)
        if opportunities
        else 0.0
    )
    return {
...
def get_crm_dashboard(db: Session):
    ...
    for stage in stages:
        count = (
            db.query(models.Opportunity)
            .filter(models.Opportunity.stage == stage)
            .count()
        )
    ...
    recent_activities = (
        db.query(models.Interaction)
        .filter(models.Interaction.created_at >= week_ago)
        .count()
    )
