...
def get_beds(db: Session, skip: int = 0, limit: int = 100):
    return (
        db.query(models.Bed)
        .filter(models.Bed.is_active == True)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def get_available_beds(db: Session, ward_id: Optional[int] = None):
    query = db.query(models.Bed).filter(
        models.Bed.is_active == True, models.Bed.status == models.BedStatus.AVAILABLE
    )
    if ward_id:
...
def get_wards(db: Session, skip: int = 0, limit: int = 100):
    return (
        ...
        .filter(models.Ward.is_active == True)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def get_bed_assignment(db: Session, assignment_id: int):
    return (
        db.query(models.BedAssignment)
        .filter(models.BedAssignment.id == assignment_id)
        .first()
    )


def get_active_bed_assignments(db: Session, patient_id: Optional[str] = None):
    query = db.query(models.BedAssignment).filter(
...
def get_bed_maintenance(db: Session, maintenance_id: int):
    return (
        db.query(models.BedMaintenance)
        .filter(models.BedMaintenance.id == maintenance_id)
        .first()
    )
...
def get_pending_maintenance(db: Session):
    return (
        db.query(models.BedMaintenance)
        .filter(models.BedMaintenance.status.in_(["scheduled", "in_progress"]))
        .all()
    )
...
def complete_maintenance(db: Session, maintenance_id: int):
    ...
    if db_maintenance:
        ...
        bed = (
            db.query(models.Bed).filter(models.Bed.id == db_maintenance.bed_id).first()
        )
        if bed:
            bed.status = models.BedStatus.AVAILABLE
            bed.updated_at = datetime.utcnow()
...
def get_bed_statistics(db: Session):
    ...
    occupied_beds = (
        db.query(models.Bed)
        .filter(
            models.Bed.is_active == True, models.Bed.status == models.BedStatus.OCCUPIED
        )
        .count()
    )
    maintenance_beds = (
