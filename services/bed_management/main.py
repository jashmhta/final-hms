...
def update_bed_status(bed_id: int, status: schemas.BedStatus, db: Session = Depends(get_db)):
...
def create_bed_assignment(assignment: schemas.BedAssignmentCreate, db: Session = Depends(get_db)):
    return crud.create_bed_assignment(db, assignment)
@app.get("/assignments/", response_model=List[schemas.BedAssignment])
def read_bed_assignments(patient_id: Optional[str] = None, db: Session = Depends(get_db)):
...
def create_bed_maintenance(maintenance: schemas.BedMaintenanceCreate, db: Session = Depends(get_db)):
