...
async def update_surgery_status(
    ...
    background_tasks.add_task(
        crud.notify_status_change, surgery_id, status_update.status
    )
...
async def get_surgical_teams(
    ...
    return crud.get_surgical_teams(
        db, specialty=specialty, available_only=available_only
    )


@app.post("/surgical-teams/", response_model=schemas.SurgicalTeam)
async def create_surgical_team(
    team: schemas.SurgicalTeamCreate, db: Session = Depends(get_db)
):
...
async def assign_surgical_team(
    ...
    background_tasks.add_task(
        crud.notify_team_assignment, surgery_id, assignment.team_id
    )
    return {
        "message": "Surgical team assigned successfully",
...
async def get_surgical_equipment(
    ...
    return crud.get_surgical_equipment(
        db, available_only=available_only, equipment_type=equipment_type
    )
...
async def assign_equipment(
    ...
    if not assignment:
        raise HTTPException(
            status_code=400, detail="Equipment not available or not found"
        )
    return {"message": "Equipment assigned successfully", "assignment": assignment}


@app.get(
    "/surgeries/{surgery_id}/equipment",
    response_model=List[schemas.EquipmentAssignment],
)
...
@app.get(
    "/surgeries/{surgery_id}/preop-checklist", response_model=schemas.PreOpChecklist
)
...
async def create_postop_notes(
    surgery_id: int, notes: schemas.PostOpNotesCreate, db: Session = Depends(get_db)
):
...
async def get_utilization_analytics(
    days: int = 30, theatre_id: Optional[int] = None, db: Session = Depends(get_db)
):
...
async def get_performance_metrics(
    days: int = 30, surgeon_id: Optional[int] = None, db: Session = Depends(get_db)
):
    return {
...
async def check_scheduling_conflicts(
    ...
    return {"conflicts_found": len(conflicts) > 0, "conflicts": conflicts}
...
async def schedule_emergency_surgery(
    ...
    if not surgery:
        raise HTTPException(
            status_code=400, detail="Unable to schedule emergency surgery"
        )
