...
@app.get(
    "/doctors/specialization/{specialization}", response_model=List[schemas.Doctor]
)
def read_doctors_by_specialization(
    specialization: schemas.DoctorSpecialization, db: Session = Depends(get_db)
):
...
def read_available_doctors(
    specialization: Optional[schemas.DoctorSpecialization] = None,
...
def create_doctor_schedule(
    schedule: schemas.DoctorScheduleCreate, db: Session = Depends(get_db)
):
...
def read_doctor_schedule(
    doctor_id: int, day_of_week: Optional[int] = None, db: Session = Depends(get_db)
):
...
def create_doctor_leave(
    leave: schemas.DoctorLeaveCreate, db: Session = Depends(get_db)
):
    return crud.create_doctor_leave(db, leave)


@app.get("/leaves/doctor/{doctor_id}", response_model=List[schemas.DoctorLeave])
def read_doctor_leaves(
    doctor_id: int, status: Optional[str] = None, db: Session = Depends(get_db)
):
    return crud.get_doctor_leaves(db, doctor_id, status)


@app.patch("/leaves/{leave_id}/approve")
def approve_doctor_leave(
    leave_id: int, approved_by: str, db: Session = Depends(get_db)
):
...
def create_doctor_performance(
    performance: schemas.DoctorPerformanceCreate, db: Session = Depends(get_db)
):
    return crud.create_doctor_performance(db, performance)


@app.get(
    "/performance/doctor/{doctor_id}", response_model=List[schemas.DoctorPerformance]
)
def read_doctor_performance(
    doctor_id: int,
