...
def get_doctor_by_doctor_id(db: Session, doctor_id_str: str):
    return db.query(models.Doctor).filter(models.Doctor.doctor_id == doctor_id_str).first()

def get_doctors(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.Doctor).filter(models.Doctor.is_active == True).offset(skip).limit(limit).all()

def get_doctors_by_specialization(db: Session, specialization: schemas.DoctorSpecialization):
    return db.query(models.Doctor).filter(
        models.Doctor.specialization == specialization,
        models.Doctor.is_active == True
    ).all()

def get_available_doctors(db: Session, specialization: Optional[schemas.DoctorSpecialization] = None):
...
def get_doctor_leaves(db: Session, doctor_id: int, status: Optional[str] = None):
    query = db.query(models.DoctorLeave).filter(models.DoctorLeave.doctor_id == doctor_id)
    if status:
...
def approve_doctor_leave(db: Session, leave_id: int, approved_by: str):
    db_leave = db.query(models.DoctorLeave).filter(models.DoctorLeave.id == leave_id).first()
    if db_leave:
...
def get_doctor_performance(db: Session, doctor_id: int, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None):
    query = db.query(models.DoctorPerformance).filter(models.DoctorPerformance.doctor_id == doctor_id)
    if start_date:
...
def create_doctor_performance(db: Session, performance: schemas.DoctorPerformanceCreate):
...
def get_doctor_dashboard(db: Session, doctor_id: int):
    ...
    # Get today's appointments (this would need to be joined with appointments table)
    today_appointments = 0  # Placeholder
    
    # Get pending consultations (this would need to be joined with consultations table)
    pending_consultations = 0  # Placeholder
    
    # Get monthly revenue
    month_start = today.replace(day=1)
    monthly_performance = db.query(models.DoctorPerformance).filter(
    ...
    # Get average patient satisfaction
    satisfaction_scores = [p.patient_satisfaction_score for p in monthly_performance if p.patient_satisfaction_score]
    avg_satisfaction = sum(satisfaction_scores) / len(satisfaction_scores) if satisfaction_scores else 0.0
    
    # Get upcoming leaves
    upcoming_leaves = db.query(models.DoctorLeave).filter(
        models.DoctorLeave.doctor_id == doctor_id,
        models.DoctorLeave.start_date > datetime.utcnow(),
        models.DoctorLeave.status == "approved"
    ).count()
    
    department = db.query(models.Department).filter(models.Department.id == doctor.department_id).first()
...
def get_doctor_statistics(db: Session):
    ...
    doctors_on_leave = db.query(models.DoctorLeave).filter(
        models.DoctorLeave.start_date <= datetime.utcnow(),
    ...
    avg_consultation_fee = sum(d.consultation_fee for d in doctors) / len(doctors) if doctors else 0.0
    
    # Get monthly revenue for all doctors
