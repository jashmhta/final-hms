...
async def upload_dicom(
    ...
    try:
        ...
        study_instance_uid = getattr(
            ds,
            "StudyInstanceUID",
            f"1.2.3.{order_id}.{int(datetime.utcnow().timestamp())}",
        )
        series_instance_uid = getattr(
            ds,
            "SeriesInstanceUID",
            f"1.2.3.{order_id}.{int(datetime.utcnow().timestamp())}.1",
        )
        sop_instance_uid = getattr(
            ds,
            "SOPInstanceUID",
            f"1.2.3.{order_id}.{int(datetime.utcnow().timestamp())}.1.1",
        )

        # Store in PACS using STOW-RS
        storage_result = await pacs_service.store_dicom(
            dicom_content, study_instance_uid
        )

        if storage_result["status"] != "success":
            raise HTTPException(
                status_code=500,
                detail=f"PACS storage failed: {storage_result.get('error', 'Unknown error')}",
            )
        ...
        try:
            hl7_order_data = {
                ...
                "procedure_code": getattr(ds, "ScheduledProcedureStepSequence", [{}])[0]
                .get("ScheduledProcedureCode", {})
                .get("CodeValue", "RAD"),
                "procedure_description": getattr(
                    ds, "StudyDescription", "Radiology Procedure"
                ),
                ...
                "clinical_indication": getattr(
                    ds, "StudyComments", "Routine examination"
                ),
            }
        ...
        raise HTTPException(
            status_code=503, detail="PACS integration service unavailable"
        )
...
def get_images(
    order_id: int, claims: dict = Depends(require_auth), db: Session = Depends(get_db)
):
    ensure_role(
        claims, {"SUPER_ADMIN", "HOSPITAL_ADMIN", "DOCTOR", "LAB_TECH", "NURSE"}
    )
    ...
    try:
        ...
        for study in studies:
            study_dict = {
                ...
                "accessed_datetime": (
                    study.accessed_datetime.isoformat()
                    if study.accessed_datetime
                    else None
                ),
            }
            ...
            try:
                ...
                study_dict["series"] = [
                    {
                        ...
                        "series_datetime": (
                            series.series_datetime.isoformat()
                            if series.series_datetime
                            else None
                        ),
                    }
                ...
                logger.warning(
                    f"Could not get series for study {study.study_instance_uid}: {e}"
                )
                study_dict["series"] = []
        ...
        logger.info(
            f"Retrieved {len(formatted_studies)} studies for patient {patient_id}"
        )
        return {
...
def send_critical_alert(order_id: int, db: Session):
    ...
    try:
        ...
        if not order or not report:
            logger.warning(
                f"Could not find order or report for critical alert: {order_id}"
            )
            return
...
async def get_study_details(
    ...
    try:
        ...
        study_details = {"study_instance_uid": study_instance_uid, "series": []}
        ...
        for series in series_list:
            try:
                ...
                logger.warning(
                    f"Could not get images for series {series.series_instance_uid}: {e}"
                )
                continue
...
@app.get(
    "/api/radiology/image/{study_instance_uid}/{series_instance_uid}/{sop_instance_uid}"
)
