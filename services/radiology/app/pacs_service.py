...
class PACSService:
    ...
    def _configure_dicom_network(self):
        ...
        try:
            ...
            self.ae.add_requested_context(build_context('1.2.840.10008.5.1.4.1.1.2'))  # CT Image Storage
            self.ae.add_requested_context(build_context('1.2.840.10008.5.1.4.1.1.4'))  # MR Image Storage
    ...
    async def query_studies(self, patient_id: str, study_date: Optional[str] = None) -> List[PACSStudy]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for study_data in studies_data:
                    study = PACSStudy(
                        study_instance_uid=study_data.get('0020000D', {}).get('Value', [''])[0],
                        ...
                        study_description=study_data.get('00081030', {}).get('Value', [''])[0],
                        patient_id=study_data.get('00100020', {}).get('Value', [''])[0],
                        patient_name=study_data.get('00100010', {}).get('Value', [{'Alphabetic': ''}])[0].get('Alphabetic', ''),
                        modalities_in_study=study_data.get('00080061', {}).get('Value', []),
                        number_of_series=study_data.get('00201206', {}).get('Value', [0])[0],
                        number_of_instances=study_data.get('00201208', {}).get('Value', [0])[0],
                        study_status=study_data.get('00550010', {}).get('Value', [''])[0],
                ...
                logger.error(f"PACS query failed: {response.status_code} - {response.text}")
                return []
    ...
    async def get_series(self, study_instance_uid: str) -> List[PACSSeries]:
        ...
        """
        try:
            response = await self._make_pacs_request('GET', f'/studies/{study_instance_uid}/series')
            ...
            if response.status_code == 200:
                ...
                for series_info in series_data:
                    series = PACSSeries(
                        series_instance_uid=series_info.get('0020000E', {}).get('Value', [''])[0],
                        series_number=series_info.get('00200011', {}).get('Value', [0])[0],
                        modality=series_info.get('00080060', {}).get('Value', [''])[0],
                        series_description=series_info.get('0008103E', {}).get('Value', [''])[0],
                        body_part_examined=series_info.get('00180015', {}).get('Value', [''])[0],
                        study_instance_uid=study_instance_uid,
                        number_of_instances=series_info.get('00201209', {}).get('Value', [0])[0],
                        series_datetime=datetime.fromisoformat(series_info.get('0008002A', {}).get('Value', [''])[0]) if series_info.get('0008002A') else None
                    )
                ...
                logger.error(f"Failed to get series: {response.status_code} - {response.text}")
                return []
    ...
    async def get_images(self, series_instance_uid: str) -> List[PACSImage]:
        ...
        """
        try:
            response = await self._make_pacs_request('GET', f'/series/{series_instance_uid}/instances')
            ...
            if response.status_code == 200:
                ...
                for instance in instances_data:
                    ...
                    metadata_response = await self._make_pacs_request('GET', f'/instances/{instance["ID"]}/metadata')
                    metadata = metadata_response.json() if metadata_response.status_code == 200 else {}

                    image = PACSImage(
                        sop_instance_uid=instance.get('ID', ''),
                        sop_class_uid=instance.get('00080016', {}).get('Value', [''])[0],
                        instance_number=instance.get('00200013', {}).get('Value', [0])[0],
                        series_instance_uid=series_instance_uid,
                        study_instance_uid=instance.get('0020000D', {}).get('Value', [''])[0],
                        ...
                        bits_allocated=metadata.get('00280100', {}).get('Value', [16])[0],
                        bits_stored=metadata.get('00280101', {}).get('Value', [16])[0],
                        high_bit=metadata.get('00280102', {}).get('Value', [15])[0],
                        photometric_interpretation=metadata.get('00280004', {}).get('Value', ['MONOCHROME2'])[0],
                        pixel_spacing=metadata.get('00280030', {}).get('Value'),
                        slice_thickness=metadata.get('00180050', {}).get('Value', [None])[0],
                        window_center=metadata.get('00281050', {}).get('Value', [None])[0],
                        window_width=metadata.get('00281051', {}).get('Value', [None])[0]
                    )
                    images.append(image)
                ...
                logger.error(f"Failed to get images: {response.status_code} - {response.text}")
                return []
    ...
    async def store_dicom(self, dicom_file: bytes, study_instance_uid: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            files = {
                'file': ('study.dcm', dicom_file, 'application/dicom')
            }
            ...
            response = await self._make_pacs_request('POST', '/studies', files=files, headers=headers)

            if response.status_code in [200, 201]:
                result = response.json()
                logger.info(f"DICOM file stored successfully: {result.get('00081199', {}).get('Value', [''])[0]}")
                return {
                    'status': 'success',
                    'study_instance_uid': study_instance_uid,
                    'response': result
                ...
                logger.error(f"Failed to store DICOM: {response.status_code} - {response.text}")
                return {
                    'status': 'error',
    ...
    async def get_wado_image(self, study_instance_uid: str, series_instance_uid: str, sop_instance_uid: str,
                           image_format: str = 'jpeg', quality: int = 85) -> bytes:
        """
        ...
        try:
            ...
            response = await self._make_pacs_request('GET', '/wado', params=params, return_raw=True)

            if response.status_code == 200:
                return response.content
            else:
                logger.error(f"Failed to get WADO image: {response.status_code}")
                raise HTTPException(status_code=response.status_code, detail="Failed to retrieve image")
    ...
    async def send_hl7_order(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = await self._make_hl7_request('POST', '/orders', json=hl7_message, headers=headers)

            if response.status_code == 200:
                result = response.json()
                logger.info(f"HL7 order sent successfully: {result.get('messageControlId')}")
                return {
                    'status': 'success',
                ...
                logger.error(f"Failed to send HL7 order: {response.status_code} - {response.text}")
                return {
                    'status': 'error',
                    'error': f"HTTP {response.status_code}: {response.text}"
                }
    ...
    async def _make_pacs_request(self, method: str, endpoint: str, params: Optional[Dict] = None,
                               json: Optional[Dict] = None, files: Optional[Dict] = None,
                               headers: Optional[Dict] = None, return_raw: bool = False):
    ...
    def _create_hl7_orm_message(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        message_control_id = hashlib.md5(str(order_data).encode()).hexdigest()[:16]
        ...
        return {
            ...
            'order': {
                ...
                'orderingFacility': order_data.get('ordering_facility', 'HMS RADIOLOGY'),
