...
def get_tasks_by_status(db: Session, status: str):
    return db.query(models.HousekeepingTask).filter(models.HousekeepingTask.status == status).all()

def get_tasks_by_priority(db: Session, priority: schemas.TaskPriority):
    return db.query(models.HousekeepingTask).filter(models.HousekeepingTask.priority == priority).all()

def get_tasks_by_staff(db: Session, staff_member: str):
    return db.query(models.HousekeepingTask).filter(models.HousekeepingTask.assigned_to == staff_member).all()
...
def update_task_status(db: Session, task_id: int, status: str, notes: Optional[str] = None):
...
def get_maintenance_request(db: Session, request_id: int):
    return db.query(models.MaintenanceRequest).filter(models.MaintenanceRequest.id == request_id).first()
...
def get_requests_by_status(db: Session, status: schemas.MaintenanceStatus):
    return db.query(models.MaintenanceRequest).filter(models.MaintenanceRequest.status == status).all()

def get_requests_by_priority(db: Session, priority: schemas.TaskPriority):
    return db.query(models.MaintenanceRequest).filter(models.MaintenanceRequest.priority == priority).all()
...
def assign_maintenance_request(db: Session, request_id: int, assigned_to: str, scheduled_date: datetime):
...
def complete_maintenance_request(db: Session, request_id: int, actual_cost: Optional[float] = None, notes: Optional[str] = None):
    db_request = db.query(models.MaintenanceRequest).filter(models.MaintenanceRequest.id == request_id).first()
    if db_request:
...
def get_equipment_by_equipment_id(db: Session, equipment_id_str: str):
    return db.query(models.Equipment).filter(models.Equipment.equipment_id == equipment_id_str).first()

def get_equipment_list(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.Equipment).filter(models.Equipment.is_active == True).offset(skip).limit(limit).all()

def get_equipment_by_status(db: Session, status: str):
    return db.query(models.Equipment).filter(
        models.Equipment.status == status,
...
def update_equipment_maintenance(db: Session, equipment_id: int, last_maintenance_date: datetime, next_maintenance_date: datetime):
...
def get_cleaning_schedule(db: Session, schedule_id: int):
    return db.query(models.CleaningSchedule).filter(models.CleaningSchedule.id == schedule_id).first()

def get_cleaning_schedules(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.CleaningSchedule).filter(models.CleaningSchedule.is_active == True).offset(skip).limit(limit).all()

def get_schedules_by_area(db: Session, area: str):
    return db.query(models.CleaningSchedule).filter(
...
def get_maintenance_statistics(db: Session):
    ...
    completed_tasks = db.query(models.HousekeepingTask).filter(models.HousekeepingTask.status == "completed").count()
    pending_tasks = db.query(models.HousekeepingTask).filter(models.HousekeepingTask.status.in_(["pending", "in_progress"])).count()
    
    # Overdue tasks (scheduled date passed but not completed)
    today = datetime.utcnow().date()
    overdue_tasks = db.query(models.HousekeepingTask).filter(
    ...
    completed_requests = db.query(models.MaintenanceRequest).filter(models.MaintenanceRequest.status == models.MaintenanceStatus.COMPLETED).count()
    pending_requests = db.query(models.MaintenanceRequest).filter(models.MaintenanceRequest.status.in_([models.MaintenanceStatus.SCHEDULED, models.MaintenanceStatus.IN_PROGRESS])).count()
    
    total_equipment = db.query(models.Equipment).filter(models.Equipment.is_active == True).count()
    operational_equipment = db.query(models.Equipment).filter(
        models.Equipment.status == "operational",
        models.Equipment.is_active == True
    ).count()
    maintenance_required = db.query(models.Equipment).filter(
...
def get_task_dashboard(db: Session):
    ...
    today_tasks = db.query(models.HousekeepingTask).filter(
        models.HousekeepingTask.scheduled_date >= today,
    ...
    completed_today = db.query(models.HousekeepingTask).filter(
        models.HousekeepingTask.completed_date >= today,
        models.HousekeepingTask.completed_date < today + timedelta(days=1)
    ).count()
    
    pending_high_priority = db.query(models.HousekeepingTask).filter(
    ...
    active_staff = db.query(models.HousekeepingTask).filter(
        models.HousekeepingTask.status == "in_progress"
    ).distinct(models.HousekeepingTask.assigned_to).count()
