...
def read_tasks_by_priority(
    priority: schemas.TaskPriority, db: Session = Depends(get_db)
):
...
def create_maintenance_request(
    request: schemas.MaintenanceRequestCreate, db: Session = Depends(get_db)
):
...
def read_requests_by_status(
    status: schemas.MaintenanceStatus, db: Session = Depends(get_db)
):
    return crud.get_requests_by_status(db, status)


@app.get(
    "/requests/priority/{priority}", response_model=List[schemas.MaintenanceRequest]
)
def read_requests_by_priority(
    priority: schemas.TaskPriority, db: Session = Depends(get_db)
):
...
def update_equipment_maintenance(
    ...
    return crud.update_equipment_maintenance(
        db, equipment_id, last_maintenance_date, next_maintenance_date
    )
...
def create_cleaning_schedule(
    schedule: schemas.CleaningScheduleCreate, db: Session = Depends(get_db)
):
    return crud.create_cleaning_schedule(db, schedule)


@app.get("/schedules/", response_model=List[schemas.CleaningSchedule])
def read_cleaning_schedules(
    skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
