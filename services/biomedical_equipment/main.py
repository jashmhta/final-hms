...
def create_equipment(
    equipment: schemas.BiomedicalEquipmentCreate, db: Session = Depends(get_db)
):
...
def read_equipment_by_status(
    status: schemas.EquipmentStatus, db: Session = Depends(get_db)
):
    return crud.get_equipment_by_status(db, status)


@app.get(
    "/equipment/category/{category}", response_model=List[schemas.BiomedicalEquipment]
)
def read_equipment_by_category(
    category: schemas.EquipmentCategory, db: Session = Depends(get_db)
):
...
@app.get(
    ...
    response_model=List[schemas.BiomedicalEquipment],
)
...
def update_equipment_status(
    equipment_id: int, status: schemas.EquipmentStatus, db: Session = Depends(get_db)
):
...
def create_maintenance(
    maintenance: schemas.EquipmentMaintenanceCreate, db: Session = Depends(get_db)
):
...
@app.get(
    ...
    response_model=List[schemas.EquipmentMaintenance],
)
...
def complete_maintenance(
    ...
    return crud.complete_maintenance(
        db, maintenance_id, findings, actions_taken, next_maintenance_date
    )


@app.post("/calibration/", response_model=schemas.EquipmentCalibration)
def create_calibration(
    calibration: schemas.EquipmentCalibrationCreate, db: Session = Depends(get_db)
):
...
@app.get(
    "/calibration/equipment/{equipment_id}",
    response_model=List[schemas.EquipmentCalibration],
)
...
def create_incident(
    incident: schemas.EquipmentIncidentCreate, db: Session = Depends(get_db)
):
...
@app.get(
    "/incidents/equipment/{equipment_id}",
    response_model=List[schemas.EquipmentIncident],
)
...
def create_training(
    training: schemas.EquipmentTrainingCreate, db: Session = Depends(get_db)
):
...
@app.get(
    "/training/equipment/{equipment_id}", response_model=List[schemas.EquipmentTraining]
)
...
@app.get(
    "/training/staff/{staff_member}", response_model=List[schemas.EquipmentTraining]
)
