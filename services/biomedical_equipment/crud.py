...
def get_equipment(db: Session, equipment_id: int):
    return db.query(models.BiomedicalEquipment).filter(models.BiomedicalEquipment.id == equipment_id).first()

def get_equipment_by_equipment_id(db: Session, equipment_id_str: str):
    return db.query(models.BiomedicalEquipment).filter(models.BiomedicalEquipment.equipment_id == equipment_id_str).first()

def get_equipment_list(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.BiomedicalEquipment).filter(models.BiomedicalEquipment.is_active == True).offset(skip).limit(limit).all()

def get_equipment_by_status(db: Session, status: schemas.EquipmentStatus):
    return db.query(models.BiomedicalEquipment).filter(
...
def update_equipment_status(db: Session, equipment_id: int, status: schemas.EquipmentStatus):
    db_equipment = db.query(models.BiomedicalEquipment).filter(models.BiomedicalEquipment.id == equipment_id).first()
    if db_equipment:
...
def get_maintenance(db: Session, maintenance_id: int):
    return db.query(models.EquipmentMaintenance).filter(models.EquipmentMaintenance.id == maintenance_id).first()

def get_maintenance_by_equipment(db: Session, equipment_id: int):
    return db.query(models.EquipmentMaintenance).filter(models.EquipmentMaintenance.equipment_id == equipment_id).all()
...
def complete_maintenance(db: Session, maintenance_id: int, findings: str, actions_taken: str, next_maintenance_date: Optional[datetime] = None):
    db_maintenance = db.query(models.EquipmentMaintenance).filter(models.EquipmentMaintenance.id == maintenance_id).first()
    if db_maintenance:
...
def get_calibration(db: Session, calibration_id: int):
    return db.query(models.EquipmentCalibration).filter(models.EquipmentCalibration.id == calibration_id).first()

def get_calibration_by_equipment(db: Session, equipment_id: int):
    return db.query(models.EquipmentCalibration).filter(models.EquipmentCalibration.equipment_id == equipment_id).all()

def get_overdue_calibrations(db: Session):
    today = datetime.utcnow().date()
    return db.query(models.EquipmentCalibration).filter(
        models.EquipmentCalibration.next_calibration_date < today
    ).all()
...
def get_incident(db: Session, incident_id: int):
    return db.query(models.EquipmentIncident).filter(models.EquipmentIncident.id == incident_id).first()

def get_incidents_by_equipment(db: Session, equipment_id: int):
    return db.query(models.EquipmentIncident).filter(models.EquipmentIncident.equipment_id == equipment_id).all()

def get_open_incidents(db: Session):
    return db.query(models.EquipmentIncident).filter(
        models.EquipmentIncident.status.in_(["open", "investigating"])
    ).all()
...
def resolve_incident(db: Session, incident_id: int, resolution: str):
    db_incident = db.query(models.EquipmentIncident).filter(models.EquipmentIncident.id == incident_id).first()
    if db_incident:
...
def get_training(db: Session, training_id: int):
    return db.query(models.EquipmentTraining).filter(models.EquipmentTraining.id == training_id).first()

def get_training_by_equipment(db: Session, equipment_id: int):
    return db.query(models.EquipmentTraining).filter(models.EquipmentTraining.equipment_id == equipment_id).all()

def get_training_by_staff(db: Session, staff_member: str):
    return db.query(models.EquipmentTraining).filter(models.EquipmentTraining.staff_member == staff_member).all()
...
def get_equipment_statistics(db: Session):
    total_equipment = db.query(models.BiomedicalEquipment).filter(models.BiomedicalEquipment.is_active == True).count()
    operational_equipment = db.query(models.BiomedicalEquipment).filter(
        models.BiomedicalEquipment.status == models.EquipmentStatus.OPERATIONAL,
        models.BiomedicalEquipment.is_active == True
    ).count()
    maintenance_required = db.query(models.BiomedicalEquipment).filter(
        models.BiomedicalEquipment.status == models.EquipmentStatus.MAINTENANCE,
        models.BiomedicalEquipment.is_active == True
    ).count()
    out_of_order = db.query(models.BiomedicalEquipment).filter(
...
def get_equipment_dashboard(db: Session):
    ...
    for status in models.EquipmentStatus:
        count = db.query(models.BiomedicalEquipment).filter(
            models.BiomedicalEquipment.status == status,
            models.BiomedicalEquipment.is_active == True
        ).count()
        status_counts[status.value] = count
    
    upcoming_maintenance = len(get_upcoming_maintenance(db, 7))  # Next 7 days
    overdue_calibrations = len(get_overdue_calibrations(db))
    recent_incidents = db.query(models.EquipmentIncident).filter(
        models.EquipmentIncident.created_at >= datetime.utcnow() - timedelta(days=30)
    ).count()
    
    # Monthly maintenance cost
    month_start = datetime.utcnow().replace(day=1, hour=0, minute=0, second=0)
    monthly_maintenance = db.query(models.EquipmentMaintenance).filter(
