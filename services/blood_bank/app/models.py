...
class Donor(TimestampedModel):
    ...
    ssn = EncryptedCharField(max_length=11, verbose_name="Social Security Number")
...
class StaffContact(models.Model):
    ...
    department = models.CharField(
        max_length=50, default="Blood Bank", verbose_name="Department"
    )
    is_active = models.BooleanField(default=True, verbose_name="Active Status")
    is_on_call = models.BooleanField(default=False, verbose_name="Currently On Call")
    receives_blood_bank_alerts = models.BooleanField(
        default=True, verbose_name="Receives Blood Bank Alerts"
    )
    receives_expiry_alerts = models.BooleanField(
        default=True, verbose_name="Receives Expiry Alerts"
    )
    receives_emergency_alerts = models.BooleanField(
        default=True, verbose_name="Receives Emergency Alerts"
    )
    ...
    def is_available_now(self):
        ...
        if self.shift_start and self.shift_end:
            if self.shift_start <= self.shift_end:
                ...
                return (
                    current_time >= self.shift_start or current_time <= self.shift_end
                )
...
class SMSLog(models.Model):
    ...
    recipient_last_4 = models.CharField(
        max_length=4, verbose_name="Recipient (Last 4 digits)"
    )
    message_preview = models.CharField(max_length=50, verbose_name="Message Preview")
    message_id = models.CharField(
        max_length=100, unique=True, verbose_name="Message ID"
    )
    ...
    message_type = models.CharField(
        max_length=30, choices=MESSAGE_TYPES, verbose_name="Message Type"
    )
    priority = models.CharField(
        max_length=10, default="normal", verbose_name="Priority"
    )
    ...
    cost = models.DecimalField(
        max_digits=6, decimal_places=4, blank=True, null=True, verbose_name="Cost"
    )
    error_code = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="Error Code"
    )
    ...
    reference_id = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="Reference ID"
    )
...
class LogEntry(models.Model):
    ...
    action = models.CharField(
        max_length=30, choices=ACTION_CHOICES, verbose_name="Action"
    )
    details = models.TextField(verbose_name="Details")
    user = models.CharField(max_length=50, blank=True, null=True, verbose_name="User")
    blood_unit_id = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="Blood Unit ID"
    )
