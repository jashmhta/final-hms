...
def get_demo_by_category(db: Session, category: str, skip: int = 0, limit: int = 100):
    ...
    return db.query(DemoModel).filter(
        DemoModel.category == category,
        DemoModel.is_active == True
...
def get_high_priority_items(db: Session, priority_threshold: int = 5):
    ...
    return db.query(DemoModel).filter(
        DemoModel.priority >= priority_threshold,
        DemoModel.is_active == True
    ).order_by(DemoModel.priority.desc()).all()
...
def get_demo_analytics(db: Session):
    ...
    category_counts = db.query(
        DemoModel.category,
        func.count(DemoModel.id).label('count')
    ).filter(DemoModel.is_active == True).group_by(DemoModel.category).all()
    ...
    ).filter(DemoModel.is_active == True).first()
    ...
    return {
        ...
        "priority_statistics": {
            "average": round(priority_stats.avg_priority, 2) if priority_stats.avg_priority else 0,
...
def create_demo_service():
    ...
    async def get_by_category(
        ...
        return create_success_response(items, f"Found {len(items)} items in category '{category}'")
    ...
    async def get_analytics(db: Session = Depends(service.get_db)):
        ...
        return create_success_response(analytics, "Demo analytics retrieved successfully")

    @app.post("/demo/batch")
    async def create_batch(
        items: list[DemoCreate],
        db: Session = Depends(service.get_db)
    ):
        ...
        return create_success_response(
            created_items,
            f"Successfully created {len(created_items)} items"
        )
...
if __name__ == "__main__":
    ...
    print("This service demonstrates the elimination of redundancy through shared libraries.")
