...
from .auth import ensure_module_enabled, ensure_role, require_auth
...
def add_quality_control(payload: QualityControlIn, db: Session = Depends(get_db)):
    ...
    if not test:
        raise HTTPException(status_code=404, detail="Test not found")
    is_within = abs(float(payload.result_value) - float(payload.target_value)) <= float(
        payload.tolerance
    )
...
async def sync_with_lis(
    ...
    try:
        ...
        return {
            ...
            "lis_server": (
                lis_config.lis_url if hasattr(lis_config, "lis_url") else "default"
            ),
        }
        ...
        raise HTTPException(
            status_code=503, detail="LIS integration service unavailable"
        )
...
def generate_automated_interpretation(value: str, test_name: str) -> str:
    ...
    try:
        ...
        if test_name_lower.startswith("glucose"):
            ...
            return (
                "Interpretation requires clinical correlation - Reference range applied"
            )
...
async def lis_status(
    ...
    try:
        ...
        return {
            ...
            "timestamp": datetime.utcnow().isoformat(),
            "message": (
                "LIS system operational"
                if connectivity_ok
                else "LIS system unreachable"
            ),
        }
...
async def create_lis_order(
    ...
    try:
        from .lis_service import LISOrder, create_lis_service
        ...
        if result["status"] == "success":
            ...
            raise HTTPException(
                status_code=500, detail=f"LIS order failed: {result.get('error')}"
            )
...
async def configure_lis(
    ...
    try:
        ...
        if not lis_url or not api_key:
            raise HTTPException(
                status_code=400, detail="LIS URL and API key are required"
            )
        ...
        if existing_config:
            ...
            new_config = LISConfig(lis_url=lis_url, api_key=api_key, enabled=True)
            db.add(new_config)
            db.commit()
        ...
        raise HTTPException(
            status_code=500, detail=f"LIS configuration error: {str(e)}"
        )
