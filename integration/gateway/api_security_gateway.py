...
from core.security_middleware import SecurityMiddleware, RateLimiter
...
class APIValidator:
    ...
    def validate_request(self, request: HttpRequest, endpoint: APIEndpoint) -> Tuple[bool, Dict]:
    ...
    def _validate_request_body(self, request: HttpRequest, endpoint: APIEndpoint) -> bool:
    ...
    def sanitize_response(self, response_data: Dict, user_permissions: List[str]) -> Dict:
...
class APIMonitor:
    ...
    def get_metrics(self, path: str = None, method: str = None, time_range: int = 3600) -> Dict:
        """Get API metrics"""
        if path and method:
        ...
        requests = [r for r in requests if datetime.fromisoformat(r['timestamp']) > cutoff_time]
    ...
    def _check_alerts(self, metrics: Dict):
        ...
        recent_metrics = self.get_metrics(metrics['path'], metrics['method'], 300)  # 5 minutes
        ...
        if recent_metrics.get('p95_response_time', 0) > self.alert_thresholds['response_time_p95']:
            log_security_event(
...
class APISecurityGateway(View):
    ...
    def dispatch(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        try:
            ...
            if endpoint.auth_required and not self._authorize_request(request, endpoint):
                return self._forbidden_response()

            # 7. Request validation
            is_valid, validation_result = self.validator.validate_request(request, endpoint)
            if not is_valid:
    ...
    def _check_cors(self, request: HttpRequest, endpoint: APIEndpoint) -> Optional[HttpResponse]:
        ...
        if origin and origin not in endpoint.allowed_origins and '*' not in endpoint.allowed_origins:
    ...
    def _add_cors_headers(self, response: HttpResponse, endpoint: APIEndpoint):
        ...
        if endpoint.allowed_origins:
            response['Access-Control-Allow-Origin'] = ', '.join(endpoint.allowed_origins)
        else:
    ...
    def _check_global_rate_limit(self, request: HttpRequest) -> bool:
        ...
        if not self.rate_limiter.check_rate_limit(
            f"global:{ip}",
            self.config.global_rate_limits['per_minute'],
            60
        ):
            return False
        ...
        if not self.rate_limiter.check_rate_limit(
            f"global_hourly:{ip}",
            self.config.global_rate_limits['per_hour'],
            3600
        ):
            return False
    ...
    def _check_endpoint_rate_limit(self, request: HttpRequest, endpoint: APIEndpoint) -> bool:
    ...
    def _forward_request(self, request: HttpRequest, endpoint: APIEndpoint) -> HttpResponse:
