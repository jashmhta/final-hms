...
class IntegrationOrchestrator:
    ...
    async def initialize(self):
        ...
        db_url = os.getenv(
            "INTEGRATION_DB_URL",
            "postgresql+asyncpg://hms:hms@localhost:5432/integration",
        )
    ...
    async def _check_integration_health(
        self, session: aiohttp.ClientSession, name: str, config: IntegrationConfig
    ):
        try:
            ...
            if (
                self.circuit_breakers[name]["failure_count"]
    ...
    async def execute_integration_request(
        ...
        if not config.enabled:
            raise HTTPException(
                status_code=503, detail=f"Integration {integration_name} is disabled"
            )
        circuit_state = self.circuit_breakers[integration_name]["state"]
        if circuit_state == "OPEN":
            raise HTTPException(
                status_code=503,
                detail=f"Integration {integration_name} is temporarily unavailable",
            )
        if not await self.check_rate_limit(integration_name):
            raise HTTPException(
                status_code=429, detail=f"Rate limit exceeded for {integration_name}"
            )
        try:
            async with aiohttp.ClientSession() as session:
        ...
        return {
            "integrations": {
                name: {
                    ...
                    "rate_limit": {
                        ...
                        "reset_time": self.rate_limiters[name][
                            "reset_time"
                        ].isoformat(),
                    },
                }
            ...
            "summary": {
                ...
                "healthy": sum(
                    ...
                    if h.status == IntegrationHealthStatus.HEALTHY
                ),
                "unhealthy": sum(
                    1
                    for h in self.health_status.values()
                    if h.status == IntegrationHealthStatus.UNHEALTHY
                ),
                "degraded": sum(
                    1
                    for h in self.health_status.values()
                    if h.status == IntegrationHealthStatus.DEGRADED
                ),
            },
        }
