...
class HL7Parser:
        ...
        try:
            ...
            if len(datetime_str) > 14:
                if '+' in datetime_str or '-' in datetime_str:
                    tz_sign = datetime_str[14] if datetime_str[14] in ['+', '-'] else None
                    if tz_sign:
    ...
    def create_acknowledgment(self, original_message: HL7Message, ack_code: HL7AckCode,
...
class HL7Processor:
    def __init__(self, orchestrator: IntegrationOrchestrator):
        ...
        self.db_url = os.getenv("HL7_DB_URL", "postgresql+asyncpg://hms:hms@localhost:5432/hl7")
        self.engine = create_async_engine(self.db_url)
        self.SessionLocal = sessionmaker(self.engine, class_=AsyncSession, expire_on_commit=False)
        self.fhir_converter = FHIRConverter()
    async def process_message(self, raw_message: str, source_system: str = "Unknown") -> Dict:
        async with self.SessionLocal() as session:
            try:
                ...
                fhir_mapping = await self.fhir_converter.convert_hl7_to_fhir(hl7_message)
                ...
                await session.commit()
                ack_message = self.parser.create_acknowledgment(hl7_message, HL7AckCode.AA)
                return {
                    "status": "success",
                    "message_control_id": hl7_message.message_control_id,
    ...
    async def _validate_message(self, message: HL7Message):
        ...
        validator = message_structure_validators.get((message.message_type, message.event_code))
        if validator:
            validator(message)
    async def _process_by_type(self, message: HL7Message, session: AsyncSession) -> Dict:
        processors = {
        ...
        if processor:
            ...
            logger.warning(f"No processor for message type: {message.message_type.value}_{message.event_code.value}")
            return {"status": "no_processor", "message": "No processor configured for this message type"}
    async def _process_adt_a01(self, message: HL7Message, session: AsyncSession) -> Dict:
        pid_segment = next((seg for seg in message.segments if seg["segment_type"] == "PID"), None)
        if not pid_segment:
    ...
    async def _process_oru_r01(self, message: HL7Message, session: AsyncSession) -> Dict:
    ...
    def _extract_patient_data(self, pid_segment: Dict) -> Dict:
        ...
        return {
            ...
            "patient_name": self._parse_patient_name(fields[5]) if len(fields) > 5 else None,
            "date_of_birth": self._parser._parse_hl7_datetime(fields[7]) if len(fields) > 7 else None,
            "gender": fields[8] if len(fields) > 8 else None,
            "address": self._parse_address(fields[11]) if len(fields) > 11 else None,
            "phone_number": self._parse_phone_number(fields[13]) if len(fields) > 13 else None,
        }
...
class FHIRConverter:
    ...
    async def _convert_adt_to_fhir(self, hl7_message: HL7Message) -> Dict:
        fhir_resources = []
        pid_segment = next((seg for seg in hl7_message.segments if seg["segment_type"] == "PID"), None)
        if pid_segment:
            patient_resource = await self._convert_pid_to_patient(pid_segment)
            fhir_resources.append(patient_resource)
        pv1_segment = next((seg for seg in hl7_message.segments if seg["segment_type"] == "PV1"), None)
        if pv1_segment:
            encounter_resource = await self._convert_pv1_to_encounter(pv1_segment, hl7_message)
    ...
    async def _convert_pid_to_patient(self, pid_segment: Dict) -> Dict:
        ...
        if len(fields) > 2 and fields[2]:
            identifiers.append({
                "system": "http://hl7.org/fhir/sid/patient-id",
                "value": fields[2]
            })
        name = {}
    ...
    async def _convert_pv1_to_encounter(self, pv1_segment: Dict, hl7_message: HL7Message) -> Dict:
        ...
        if len(fields) > 3 and fields[3]:
            encounter_resource["participant"] = [{
                ...
                "individual": {
                    "reference": f"Practitioner/{fields[3]}"
                }
            }]
        return encounter_resource
class MLLPServer:
    def __init__(self, host: str = "0.0.0.0", port: int = 2575, processor: HL7Processor = None):
        self.host = host
        self.port = port
    ...
    async def handle_client(self, client_socket: socket.socket, address: Tuple[str, int]):
        ...
        try:
            ...
            if message_end != -1:
                ...
                if self.processor:
                    result = await self.processor.process_message(message_data, f"MLLP:{address[0]}")
