...
class DataSynchronizer:
    def __init__(self, orchestrator: IntegrationOrchestrator):
        ...
        self.db_url = os.getenv("SYNC_DB_URL", "postgresql+asyncpg://hms:hms@localhost:5432/sync")
    ...
    def _initialize_handlers(self):
        ...
        self.entity_handlers[SyncEntityType.RADIOLOGY_ORDER] = self._handle_radiology_sync
    def _initialize_conflict_resolvers(self):
        self.conflict_resolvers[ConflictResolutionStrategy.SOURCE_WINS] = self._resolve_source_wins
        self.conflict_resolvers[ConflictResolutionStrategy.TARGET_WINS] = self._resolve_target_wins
        self.conflict_resolvers[ConflictResolutionStrategy.MERGE] = self._resolve_merge
        self.conflict_resolvers[ConflictResolutionStrategy.TIMESTAMP_BASED] = self._resolve_timestamp_based
        self.conflict_resolvers[ConflictResolutionStrategy.FIELD_LEVEL_MERGE] = self._resolve_field_level_merge
    async def publish_event(self, event: SyncEvent) -> str:
        try:
            ...
            logger.info(f"Published sync event: {event.event_id} - {event.event_type.value} {event.entity_type.value}")
        ...
        if entity_type in required_fields:
            for field in required_fields[entity_type]:
                if field not in data:
                    raise ValueError(f"Required field '{field}' missing for {entity_type.value}")
        ...
        try:
            ...
            for target_system in event.target_systems:
                try:
                    ...
                    result.errors.append(f"Failed to sync with {target_system}: {str(e)}")
    ...
    async def _sync_patient_create(self, event: SyncEvent, target_system: str):
        exists = await self._check_entity_exists(target_system, event.entity_type, event.entity_id)
        if exists:
            conflict = await self._detect_conflict(event, target_system, "Patient already exists")
            if conflict:
                await self._handle_conflict(event, target_system, conflict)
        else:
            await self._create_entity_in_target(target_system, event.entity_type, event.data)
    async def _sync_patient_update(self, event: SyncEvent, target_system: str):
        target_version = await self._get_entity_version(target_system, event.entity_type, event.entity_id)
        source_version = event.metadata.get("version")
        if target_version and target_version != source_version:
            conflict = await self._detect_conflict(event, target_system, "Version mismatch")
            if conflict:
                await self._handle_conflict(event, target_system, conflict)
        else:
            await self._update_entity_in_target(target_system, event.entity_type, event.entity_id, event.data)
    async def _sync_patient_delete(self, event: SyncEvent, target_system: str):
        await self._delete_entity_in_target(target_system, event.entity_type, event.entity_id)
    async def _check_entity_exists(self, target_system: str, entity_type: SyncEntityType, entity_id: str) -> bool:
        try:
            ...
            return False
    async def _get_entity_version(self, target_system: str, entity_type: SyncEntityType, entity_id: str) -> Optional[str]:
        try:
            response = await self.orchestrator.execute_integration_request(
                target_system,
                "GET",
            ...
            return None
    async def _create_entity_in_target(self, target_system: str, entity_type: SyncEntityType, data: Dict):
        await self.orchestrator.execute_integration_request(
            target_system,
            "POST",
            f"/api/{entity_type.value.lower()}",
            data=data
        )
    async def _update_entity_in_target(self, target_system: str, entity_type: SyncEntityType, entity_id: str, data: Dict):
        await self.orchestrator.execute_integration_request(
            target_system,
            "PUT",
            f"/api/{entity_type.value.lower()}/{entity_id}",
    ...
    async def _delete_entity_in_target(self, target_system: str, entity_type: SyncEntityType, entity_id: str):
        await self.orchestrator.execute_integration_request(
            target_system,
            "DELETE",
            f"/api/{entity_type.value.lower()}/{entity_id}"
        )
    async def _detect_conflict(self, event: SyncEvent, target_system: str, conflict_type: str) -> Optional[Dict]:
        conflict = {
            "event_id": event.event_id,
            "entity_type": event.entity_type.value,
            "entity_id": event.entity_id,
        ...
        return conflict
    async def _handle_conflict(self, event: SyncEvent, target_system: str, conflict: Dict):
        resolver = self.conflict_resolvers.get(event.conflict_resolution)
        if resolver:
            await resolver(event, target_system, conflict)
        else:
            logger.warning(f"No conflict resolver for {event.conflict_resolution.value}")
    async def _resolve_source_wins(self, event: SyncEvent, target_system: str, conflict: Dict):
        if event.event_type == SyncEventType.UPDATE:
            await self._update_entity_in_target(target_system, event.entity_type, event.entity_id, event.data)
        elif event.event_type == SyncEventType.CREATE:
            await self._create_entity_in_target(target_system, event.entity_type, event.data)
    async def _resolve_target_wins(self, event: SyncEvent, target_system: str, conflict: Dict):
        pass
    async def _resolve_merge(self, event: SyncEvent, target_system: str, conflict: Dict):
        target_data = await self._get_entity_data(target_system, event.entity_type, event.entity_id)
        merged_data = {**target_data, **event.data}
        await self._update_entity_in_target(target_system, event.entity_type, event.entity_id, merged_data)
    async def _resolve_timestamp_based(self, event: SyncEvent, target_system: str, conflict: Dict):
        source_timestamp = event.metadata.get("updated_at")
        target_timestamp = await self._get_entity_timestamp(target_system, event.entity_type, event.entity_id)
    ...
    async def _resolve_field_level_merge(self, event: SyncEvent, target_system: str, conflict: Dict):
        pass
    async def _get_entity_data(self, target_system: str, entity_type: SyncEntityType, entity_id: str) -> Dict:
        response = await self.orchestrator.execute_integration_request(
            target_system,
            "GET",
            f"/api/{entity_type.value.lower()}/{entity_id}"
    ...
    async def _get_entity_timestamp(self, target_system: str, entity_type: SyncEntityType, entity_id: str) -> Optional[datetime]:
        response = await self.orchestrator.execute_integration_request(
            target_system,
            "GET",
    ...
    async def _update_metrics(self, event: SyncEvent, result: SyncResult):
        for target_system in event.target_systems:
            metrics_key = f"{event.source_system}:{target_system}:{event.entity_type.value}"
            if metrics_key not in self.metrics_cache:
                self.metrics_cache[metrics_key] = await self._load_metrics(event.source_system, target_system, event.entity_type)
            ...
            total_time = metrics.avg_processing_time * (metrics.total_events - 1) + result.processing_time
            metrics.avg_processing_time = total_time / metrics.total_events
            metrics.last_event_time = datetime.utcnow()
            metrics.updated_at = datetime.utcnow()
            await session.commit()
    async def _load_metrics(self, source_system: str, target_system: str, entity_type: SyncEntityType) -> SyncMetrics:
        async with self.SessionLocal() as session:
    ...
    async def connect_websocket(self, websocket: WebSocket, client_id: str, entity_types: List[str] = None):
        ...
        async with self.SessionLocal() as session:
            ...
            if event_log:
                return {
                    ...
                    "processed_at": event_log.processed_at.isoformat() if event_log.processed_at else None
                }
        return None
    async def get_sync_metrics(self, source_system: str = None, target_system: str = None,
                             entity_type: str = None, time_range: int = 24) -> List[Dict]:
        async with self.SessionLocal() as session:
...
async def get_sync_metrics(
    ...
    return await synchronizer.get_sync_metrics(source_system, target_system, entity_type, time_range)
