...
class DICOMServerConfig:
    ...
    supported_transfer_syntaxes: List[str] = field(
...
class DICOMConfigurationManager:
    def __init__(self, config_file: Optional[str] = None):
        self.config_file = config_file or os.getenv(
            "DICOM_CONFIG_FILE", "/etc/dicom/config.yaml"
        )
    ...
    def _parse_config_data(self, config_data: Dict) -> DICOMFullConfig:
        ...
        if "server" in config_data:
            ...
            config.server = DICOMServerConfig(
                ...
                supported_transfer_syntaxes=server_data.get(
                    "supported_transfer_syntaxes",
                    config.server.supported_transfer_syntaxes,
                ),
                supported_sop_classes=server_data.get(
                    "supported_sop_classes", config.server.supported_sop_classes
                ),
                enabled_services=server_data.get(
                    "enabled_services", config.server.enabled_services
                ),
            )
        if "storage" in config_data:
            ...
            config.storage = DICOMStorageConfig(
                ...
                compression=ImageCompression(
                    storage_data.get("compression", "JPEG2000")
                ),
                quality=ImageQuality(storage_data.get("quality", "HIGH")),
                storage_policy=StoragePolicy(
                    storage_data.get("storage_policy", "OPTIMIZED")
                ),
        ...
        if "security" in config_data:
            ...
            config.security = DICOMSecurityConfig(
                security_level=DICOMSecurityLevel(
                    security_data.get("security_level", "RESTRICTED")
                ),
                ...
                allowed_ip_ranges=security_data.get(
                ...
                audit_log_retention_days=security_data.get(
                    "audit_log_retention_days", 90
                ),
                tls_enabled=security_data.get("tls_enabled", True),
                tls_cert_path=security_data.get(
                    "tls_cert_path", "/etc/ssl/certs/dicom.crt"
                ),
                tls_key_path=security_data.get(
                    "tls_key_path", "/etc/ssl/private/dicom.key"
                ),
        ...
        if "integration" in config_data:
            ...
            config.integration = DICOMIntegrationConfig(
                ...
                fhir_server_url=integration_data.get(
                    "fhir_server_url", "http://localhost:8080/fhir"
                ),
                hl7_integration=integration_data.get("hl7_integration", True),
                hl7_server_url=integration_data.get(
                    "hl7_server_url", "http://localhost:8081"
                ),
                ...
                enable_pacs_integration=integration_data.get(
                    "enable_pacs_integration", True
                ),
                pacs_servers=integration_data.get("pacs_servers", []),
                enable_ris_integration=integration_data.get(
                    "enable_ris_integration", True
                ),
                ris_server_url=integration_data.get(
                    "ris_server_url", "http://localhost:8085"
                ),
                enable_lis_integration=integration_data.get(
                    "enable_lis_integration", True
                ),
                lis_server_url=integration_data.get(
                    "lis_server_url", "http://localhost:8083"
                ),
                enable_his_integration=integration_data.get(
                    "enable_his_integration", True
                ),
                his_server_url=integration_data.get(
                    "his_server_url", "http://localhost:8000"
                ),
                enable_emr_integration=integration_data.get(
                    "enable_emr_integration", True
                ),
                emr_server_url=integration_data.get(
                    "emr_server_url", "http://localhost:8002"
                ),
                enable_viewer_integration=integration_data.get(
                    "enable_viewer_integration", True
                ),
                viewer_url=integration_data.get("viewer_url", "http://localhost:8086"),
                enable_ai_integration=integration_data.get(
                    "enable_ai_integration", True
                ),
                ai_server_url=integration_data.get(
                    "ai_server_url", "http://localhost:8087"
                ),
            )
        if "performance" in config_data:
            performance_data = config_data["performance"]
            config.performance = DICOMPerformanceConfig(
                max_concurrent_connections=performance_data.get(
                    "max_concurrent_connections", 100
                ),
                max_concurrent_transfers=performance_data.get(
                ...
                enable_async_processing=performance_data.get(
                ...
                enable_load_balancing=performance_data.get(
                    "enable_load_balancing", True
                ),
                load_balancer_strategy=performance_data.get(
        ...
        if "monitoring" in config_data:
            ...
            config.monitoring = DICOMMonitoringConfig(
                ...
                enable_metrics_collection=monitoring_data.get(
                    "enable_metrics_collection", True
                ),
                metrics_collection_interval=monitoring_data.get(
                    "metrics_collection_interval", 60
                ),
                enable_performance_monitoring=monitoring_data.get(
                    "enable_performance_monitoring", True
                ),
                enable_error_tracking=monitoring_data.get(
                ...
                enable_real_time_monitoring=monitoring_data.get(
                    "enable_real_time_monitoring", True
                ),
                monitoring_dashboard_url=monitoring_data.get(
                    "monitoring_dashboard_url", "http://localhost:3000"
                ),
            )
        return config
    ...
    def get_environment_config(self) -> Dict:
        return {
            ...
            "DICOM_DB_URL": os.getenv(
                "DICOM_DB_URL", "postgresql+asyncpg://hms:hms@localhost:5432/dicom"
            ),
    ...
    def validate_configuration(self) -> bool:
        try:
            if not os.path.exists(self.config.storage.base_path):
                logger.warning(
                    f"Storage path does not exist: {self.config.storage.base_path}"
                )
            ...
            if self.config.security.tls_enabled:
                if not os.path.exists(self.config.security.tls_cert_path):
                    logger.warning(
                        f"TLS certificate file not found: {self.config.security.tls_cert_path}"
                    )
                if not os.path.exists(self.config.security.tls_key_path):
                    logger.warning(
                        f"TLS key file not found: {self.config.security.tls_key_path}"
                    )
            if not (0.1 <= self.config.storage.compression_ratio <= 1.0):
                raise ValueError(
                    f"Invalid compression ratio: {self.config.storage.compression_ratio}"
                )
            if self.config.security.connection_timeout <= 0:
                raise ValueError(
                    f"Invalid connection timeout: {self.config.security.connection_timeout}"
                )
            if self.config.performance.max_concurrent_connections <= 0:
                raise ValueError(
                    f"Invalid max concurrent connections: {self.config.performance.max_concurrent_connections}"
                )
