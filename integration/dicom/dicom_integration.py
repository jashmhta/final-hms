...
DICOMStudy.series = relationship(
    "DICOMSeries", order_by=DICOMSeries.series_number, back_populates="study"
)
DICOMSeries.instances = relationship(
    "DICOMInstance", order_by=DICOMInstance.instance_number, back_populates="series"
)
...
class DICOMIntegration:
    ...
    async def store_dicom_image(
        self, dicom_file: BinaryIO, metadata: Optional[Dict] = None
    ) -> Dict:
        try:
            ...
            if self.storage_config.enable_deduplication:
                ...
                if existing_instance:
                    logger.info(
                        f"Duplicate image found, using existing: {existing_instance.sop_instance_uid}"
                    )
                    return existing_instance.dict()
            await self._store_file(dicom_file, file_paths["original"])
            compressed_path = await self._create_compressed_version(
                dicom_data, file_paths["compressed"]
            )
            thumbnail_path = await self._create_thumbnail(
                dicom_data, file_paths["thumbnail"]
            )
            async with self.SessionLocal() as session:
                study = await self._get_or_create_study(session, dicom_metadata)
                series = await self._get_or_create_series(
                    session, dicom_metadata, study
                )
                instance = DICOMInstance(
                    ...
                    file_size_mb=os.path.getsize(file_paths["original"])
                    / (1024 * 1024),
                ...
                logger.info(
                    f"Successfully stored DICOM image: {dicom_metadata.sop_instance_uid}"
                )
                return {
                    "sop_instance_uid": dicom_metadata.sop_instance_uid,
                    "study_instance_uid": dicom_metadata.study_instance_uid,
    ...
    def _extract_metadata(self, dicom_data: Dataset) -> DICOMMetadata:
        return DICOMMetadata(
            ...
            window_center=(
                float(getattr(dicom_data, "WindowCenter", 0))
                if hasattr(dicom_data, "WindowCenter")
                else None
            ),
            window_width=(
                float(getattr(dicom_data, "WindowWidth", 0))
            ...
            performing_physician_name=getattr(
                dicom_data, "PerformingPhysicianName", None
            ),
            referring_physician_name=getattr(
                dicom_data, "ReferringPhysicianName", None
            ),
        )
    ...
    def _generate_file_paths(self, metadata: DICOMMetadata) -> Dict[str, Path]:
        ...
        return {
            ...
            "compressed": self.storage_paths["compressed"]
            / metadata.study_instance_uid
            / metadata.series_instance_uid
            / f"{metadata.sop_instance_uid}.jpg",
            "thumbnail": self.storage_paths["thumbnails"]
            / metadata.study_instance_uid
            / metadata.series_instance_uid
            / f"{metadata.sop_instance_uid}_thumb.jpg",
        }
    ...
    async def _create_compressed_version(
        self, dicom_data: Dataset, output_path: Path
    ) -> str:
        try:
            output_path.parent.mkdir(parents=True, exist_ok=True)
            pixel_array = dicom_data.pixel_array
            if hasattr(dicom_data, "WindowCenter") and hasattr(
                dicom_data, "WindowWidth"
            ):
                window_center = (
                    float(dicom_data.WindowCenter)
                    if isinstance(dicom_data.WindowCenter, (int, float))
                    else float(dicom_data.WindowCenter[0])
                )
                window_width = (
                    float(dicom_data.WindowWidth)
                    if isinstance(dicom_data.WindowWidth, (int, float))
                    else float(dicom_data.WindowWidth[0])
                )
    ...
    async def _get_or_create_study(
        self, session: AsyncSession, metadata: DICOMMetadata
    ) -> DICOMStudy:
        result = await session.execute(
            session.query(DICOMStudy).filter(
                DICOMStudy.study_instance_uid == metadata.study_instance_uid
            )
        )
    ...
    async def _get_or_create_series(
        self, session: AsyncSession, metadata: DICOMMetadata, study: DICOMStudy
    ) -> DICOMSeries:
        result = await session.execute(
            session.query(DICOMSeries).filter(
                DICOMSeries.series_instance_uid == metadata.series_instance_uid
            )
        )
        ...
        try:
            async with self.SessionLocal() as session:
                ...
                if "patient_id" in query_params:
                    query = query.filter(
                        DICOMStudy.patient_id.ilike(f"%{query_params['patient_id']}%")
                    )
                if "patient_name" in query_params:
                    query = query.filter(
                        DICOMStudy.patient_name.ilike(
                            f"%{query_params['patient_name']}%"
                        )
                    )
                if "study_date" in query_params:
                    query = query.filter(
                        DICOMStudy.study_date == query_params["study_date"]
                    )
                if "modality" in query_params:
                    query = query.filter(
                        DICOMStudy.modalities_in_study.contains(
                            [query_params["modality"]]
                        )
                    )
                if "accession_number" in query_params:
                    query = query.filter(
                        DICOMStudy.accession_number == query_params["accession_number"]
                    )
    ...
    async def get_dicom_image(
        self, sop_instance_uid: str, quality: ImageQuality = ImageQuality.ORIGINAL
    ) -> StreamingResponse:
        try:
            async with self.SessionLocal() as session:
                result = await session.execute(
                    session.query(DICOMInstance).filter(
                ...
                if quality == ImageQuality.THUMBNAIL and instance.thumbnail_path:
                    ...
                    quality
                    in [ImageQuality.HIGH, ImageQuality.MEDIUM, ImageQuality.LOW]
                    and instance.compressed_path
                ):
    ...
    async def anonymize_dicom_image(self, sop_instance_uid: str) -> Dict:
        try:
            async with self.SessionLocal() as session:
                ...
                compressed_path = await self._create_compressed_version(
                    anonymized_data, new_file_paths["compressed"]
                )
                thumbnail_path = await self._create_thumbnail(
                    anonymized_data, new_file_paths["thumbnail"]
                )
                ...
                logger.info(
                    f"Successfully anonymized DICOM image: {sop_instance_uid} -> {new_sop_uid}"
                )
                return {
    ...
    async def _log_query(
        self, query_type: str, query_parameters: Dict, result_count: int
    ):
        async with self.SessionLocal() as session:
