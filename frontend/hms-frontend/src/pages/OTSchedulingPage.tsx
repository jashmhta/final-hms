...
const OTSchedulingPage: React.FC = () => {
  ...
  const hospital =
    JSON.parse(atob((token.split(".")[1] || "") + "=="))?.hospital || 1;

  const fetchSlots = useCallback(async () => {
    const res = await fetch(`/api/ot/slots?hospital_id=${hospital}`, {
  ...
  const createSlot = async () => {
    ...
    const res = await fetch("/api/ot/slots", {
      method: "POST",
      headers: {
        ...
        {slots.map((s) => (
          <Paper key={s.id} variant="outlined" sx={{ p: 1.5 }}>
            <Typography variant="subtitle2">
              {s.theatre} • {new Date(s.start_at).toLocaleString()} →{" "}
