  ...
  const send = async () => {
    ...
    try {
      await axios.post("/api/audit/events", {
        ...
        resource_id: String(r.data.id),
        detail: `Sent to ${form.recipient}`,
      });
    } catch {
      /* Audit logging failed, but notification was sent */
    }
    await loadHistory();
  };
      ...
      <Paper sx={{ p: 2, mt: 2 }}>
        ...
        {history.map((h) => (
          <Box key={h.id} sx={{ mb: 1 }}>
            <Typography>
              #{h.id} {h.channel} to {h.recipient} - {h.subject} [{h.status}]
            </Typography>
          </Box>
        ))}
      </Paper>
