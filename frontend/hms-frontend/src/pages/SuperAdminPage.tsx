...
const SuperAdminPage: React.FC = () => {
  ...
  const [subsByHospital, setSubsByHospital] = useState<
    Record<number, HospitalPlan | null>
  >({});
  ...
  useEffect(() => {
    ...
    hospitals.forEach((h) => {
      axios
        .get("/api/hospital-plans/", { params: { hospital: h.id } })
        .then((r) => {
          const sub = (Array.isArray(r.data) ? r.data[0] : r.data) as
            | HospitalPlan
            | undefined;
            ...
            <Paper key={h.id} variant="outlined" sx={{ p: 2 }}>
              ...
              >
                ...
                <Select
                  size="small"
                  value={sub?.plan?.id || ""}
                  onChange={(e) => changePlan(h.id, Number(e.target.value))}
                  sx={{ minWidth: 180 }}
                >
                  {plans.map((p) => (
                  ...
                  <FormControlLabel
                    ...
                    control={
                      <Switch
                        checked={
                          sub &&
                          (sub[f.key] ?? (sub.plan ? sub.plan[f.key] : true))
                            ? true
                            : false
                        }
                        onChange={(_e, checked) =>
                          updateSub(h.id, { [f.key]: checked })
                        }
                      />
                    }
                    label={f.label}
