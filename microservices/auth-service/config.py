...
class Settings(BaseSettings):
    ...
    JWT_SECRET_KEY: str = Field(default=secrets.token_urlsafe(32), env="JWT_SECRET_KEY")
    JWT_REFRESH_SECRET_KEY: str = Field(
        default=secrets.token_urlsafe(32), env="JWT_REFRESH_SECRET_KEY"
    )
    ...
    ACCOUNT_LOCKOUT_DURATION: int = Field(
        default=30, env="ACCOUNT_LOCKOUT_DURATION"
    )  # minutes
    ...
    EMAIL_SERVICE_URL: str = Field(
        default="http://email-service:8002", env="EMAIL_SERVICE_URL"
    )
    ...
    RATE_LIMIT_PASSWORD_RESET: int = Field(
        default=2, env="RATE_LIMIT_PASSWORD_RESET"
    )  # per hour
