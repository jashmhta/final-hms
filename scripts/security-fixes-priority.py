...
class SecurityVulnerabilityScanner:
    ...
    def scan_directory(self, directory: str) -> List[Dict]:
        ...
        skip_patterns = [
            ...
            'migrations', 'static', 'media', 'venv', 'env'
        ]
        ...
        for root, dirs, files in os.walk(directory):
            ...
            for file in files:
                if file.endswith(('.py', '.yml', '.yaml', '.json', '.env', '.js', '.ts')):
                ...
                try:
                    ...
                    for vuln in safety_report:
                        vulnerabilities.append({
                            ...
                            'vulnerable_version': vuln.get('installed_version', 'unknown'),
    ...
    def prioritize_fixes(self, vulnerabilities: List[Dict]) -> Dict:
        ...
        prioritized['critical'].sort(key=lambda x: (
    ...
    def generate_fix_commands(self, vulnerabilities: List[Dict]) -> List[str]:
        ...
        for vuln in vulnerabilities:
            if vuln['type'] == 'hardcoded_passwords':
                commands.append(f"sed -i 's/{vuln['content']}/${{ENV_VAR}}/g' {vuln['file']}")
                ...
                commands.append(f"sed -i 's/DEBUG = True/DEBUG = False/g' {vuln['file']}")
    ...
    def run_scan(self, output_file: str = None):
        ...
        report = {
            ...
            'fix_commands': self.generate_fixes(prioritized['critical'] + prioritized['high']),
        }
    ...
    def generate_fixes(self, vulnerabilities: List[Dict]) -> List[str]:
        ...
        for vuln in vulnerabilities[:10]:  # Top 10 fixes
            ...
            if vuln['type'] == 'hardcoded_passwords':
                fixes.append(f"sed -i 's/{vuln['content'].split('=')[1]}/${{SECRET_NAME}}/g' {vuln['file']}")
            elif vuln['type'] == 'default_credentials':
