...
class CDNDeployer:
        ...
        try:
            ...
            for asset_path, asset_info in self.manifest.get('assets', {}).items():
                ...
                if os.path.exists(gzipped_path):
                    s3.upload_file(
                        gzipped_path, bucket, f'{s3_key}.gz',
                        ExtraArgs={
                            'ContentType': content_type,
    ...
    def invalidate_cloudfront(self):
        ...
        try:
            ...
            if self._wait_for_invalidation(cloudfront, distribution_id, invalidation_id):
    ...
    def _wait_for_invalidation(self, cloudfront, distribution_id, invalidation_id, timeout=300):
    ...
    def run_health_checks(self):
        ...
        for asset_path, asset_info in self.manifest.get('assets', {}).items():
            if asset_info.get('cdn_url'):
                url = asset_info['cdn_url'] if asset_info['cdn_url'].startswith('http') else f"{base_url}{asset_info['cdn_url']}"
                total_count += 1
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.warning(f"Health check failed for {url}: {response.status_code}")
                except Exception as e:
                    logger.error(f"Health check error for {url}: {e}")

        success_rate = (success_count / total_count * 100) if total_count > 0 else 0
        logger.info(f"Health check success rate: {success_rate:.1f}% ({success_count}/{total_count})")
    ...
    def _send_slack_notification(self, notification):
        ...
        message = {
            ...
            'attachments': [{
                'color': color,
                'fields': [
                    {'title': 'Status', 'value': 'Success' if notification['success'] else 'Failed', 'short': True},
                    {'title': 'Environment', 'value': notification['environment'], 'short': True},
                    {'title': 'Assets Deployed', 'value': str(notification['assets_deployed']), 'short': True},
                    {'title': 'Version', 'value': notification['version'], 'short': True},
