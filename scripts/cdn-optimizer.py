...
class CDNOptimizer:
    ...
    def optimize_static_assets(self) -> Dict[str, Any]:
        ...
        logger.info(f"Optimization complete: {results['optimized_files']} files, "
            ...
            for file_path in file_paths:
                if file_path.is_file() and not file_path.name.startswith('.'):
                    ...
                    if image_result:
                        ...
                        results['total_size_compressed'] += image_result['compressed_size']
    ...
    def _optimize_image(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_path.suffix.lower() in ['.jpg', '.jpeg']:
                ...
                subprocess.run([
                    'optipng', '-o7', '-quiet',
                    '-out', str(optimized_path),
                    str(file_path)
                ], check=False, capture_output=True)
            elif file_path.suffix.lower() == '.svg':
                # Optimize SVG
                subprocess.run([
                    'svgo', '--input', str(file_path),
            ...
            if optimized_path.exists():
                ...
                if optimized_size < original_size:
                    ...
                    return {
                        ...
                        'hash': hashlib.md5(file_path.read_bytes()).hexdigest()[:8]
                    }
            ...
            try:
                ...
                if file_info.get('compressed'):
                    ...
                    if os.path.exists(compressed_path):
                        s3.upload_file(
                            compressed_path, bucket, file_info['compressed'],
                            ExtraArgs={
                                'ContentType': self._get_content_type(original_path),
                ...
                invalidation = cloudfront.create_invalidation(
                    ...
                    InvalidationBatch={
                        'Paths': {
                            ...
                            'Items': [f'/{info["hashed_path"]}' for info in manifest['files'].values()]
                        },
                        'CallerReference': str(datetime.now().timestamp())
                    }
                )
                results['invalidated_paths'] = list(manifest['files'].keys())
                logger.info(f"Created CloudFront invalidation: {invalidation['Invalidation']['Id']}")
            except ClientError as e:
        ...
        if provider == 'aws':
            ...
            if distribution_id:
                try:
                    ...
                    logger.info(f"Invalidated {len(paths)} paths: {invalidation['Invalidation']['Id']}")
...
def main():
    ...
    parser.add_argument('--optimize', action='store_true', help='Optimize static assets')
