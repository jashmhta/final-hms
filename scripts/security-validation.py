...
from backend.core.compliance_automation import ComplianceAutomationEngine, ComplianceDashboard
...
class SecurityValidator:
    ...
    def _validate_compliance(self):
        ...
        print(f"   ✓ HIPAA Compliance: {self.results['compliance']['hipaa'].get('overall_status', 'NOT_ASSESSED')}")
        print(f"   ✓ GDPR Compliance: {self.results['compliance']['gdpr'].get('overall_status', 'NOT_ASSESSED')}")
        print(f"   ✓ PCI DSS Compliance: {self.results['compliance']['pci_dss'].get('overall_status', 'NOT_ASSESSED')}")
    ...
    def _validate_security_controls(self):
        ...
        print(f"   ✓ Encryption Controls: {'✅' if controls['encryption']['compliant'] else '❌'}")
        print(f"   ✓ Authentication: {'✅' if controls['authentication']['compliant'] else '❌'}")
        print(f"   ✓ Authorization: {'✅' if controls['authorization']['compliant'] else '❌'}")
        print(f"   ✓ Audit Logging: {'✅' if controls['audit_logging']['compliant'] else '❌'}")
        print(f"   ✓ Backup Controls: {'✅' if controls['backup']['compliant'] else '❌'}")
        print(f"   ✓ Patch Management: {'✅' if controls['patch_management']['compliant'] else '❌'}")
    ...
    def _validate_network_security(self):
        ...
        print(f"   ✓ Firewall Rules: {'✅' if network_security['firewall_rules']['enabled'] else '❌'}")
        print(f"   ✓ TLS Version: {network_security['tls_version']['version']}")
        print(f"   ✓ WAF Enabled: {'✅' if network_security['waf_enabled']['enabled'] else '❌'}")
        print(f"   ✓ Network Segmentation: {'✅' if network_security['network_segmentation']['enabled'] else '❌'}")
        print(f"   ✓ DDoS Protection: {'✅' if network_security['ddos_protection']['enabled'] else '❌'}")
    ...
    def _validate_access_controls(self):
        ...
        print(f"   ✓ MFA Enforced: {'✅' if access_controls['mfa_enforced']['enforced'] else '❌'}")
        print(f"   ✓ Password Policy: {'✅' if access_controls['password_policy']['compliant'] else '❌'}")
        print(f"   ✓ Session Timeout: {access_controls['session_timeout']['timeout']} minutes")
        print(f"   ✓ Concurrent Sessions: {'✅' if access_controls['concurrent_sessions']['limited'] else '❌'}")
        print(f"   ✓ Privilege Access: {'✅' if access_controls['privilege_access']['controlled'] else '❌'}")
    ...
    def _calculate_overall_score(self):
        ...
        scores['compliance'] = 100 if compliance_status == 'COMPLIANT' else 50 if compliance_status == 'PARTIALLY_COMPLIANT' else 0

        # Security controls score
        compliant_controls = sum(1 for c in self.results['security_controls'].values() if c.get('compliant', False))
        scores['security_controls'] = (compliant_controls / len(self.results['security_controls'])) * 100
        ...
        enabled_controls = sum(1 for c in self.results['network_security'].values() if c.get('enabled', True))
        scores['network_security'] = (enabled_controls / len(self.results['network_security'])) * 100
        ...
        self.results['vulnerabilities']['total'] = sum(self.results['vulnerabilities'].values())
    ...
    def _generate_recommendations(self):
        ...
        if not all(c.get('enabled', True) for c in self.results['network_security'].values()):
            recommendations.append({
                'priority': 'MEDIUM',
    ...
    def generate_report(self, output_file: str = None):
        ...
        report = {
            ...
            'summary': {
                ...
                'status': 'SECURE' if self.results['overall_security_score'] >= 90 else 'NEEDS_IMPROVEMENT',
...
def main():
    ...
    print(f"Status: {'✅ SECURE' if results['overall_security_score'] >= 90 else '⚠️ NEEDS IMPROVEMENT'}")
