...
class SecurityTester:
    ...
    async def test_data_privacy(self):
        ...
        for test in privacy_tests:
            protection_methods = test.get('encryption_methods', []) or test.get('masking_techniques', []) or test.get('anonymization_methods', []) or test.get('consent_features', []) or test.get('minimization_practices', []) or test.get('pii_protection', []) or test.get('disposal_methods', []) or test.get('enforcement_mechanisms', []) or test.get('access_controls', []) or test.get('assessment_procedures', [])
    ...
    async def test_authentication(self):
        ...
        for test in auth_tests:
            auth_features = test.get('requirements', []) or test.get('mfa_methods', []) or test.get('session_features', []) or test.get('token_features', []) or test.get('oauth_features', [])
    ...
    async def test_authorization(self):
        ...
        for test in authz_tests:
            authz_features = test.get('rbac_features', []) or test.get('privilege_features', []) or test.get('hierarchy_features', []) or test.get('access_features', []) or test.get('separation_features', [])
