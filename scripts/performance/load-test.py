...
class PerformanceTester:
        ...
        try:
            async with self.session.request(
                ...
                if response_time > 1.0:
                    logger.warning(f"Slow response: {method} {url} - {response_time:.2f}s")
    ...
    async def user_scenario(self, user_id: int):
        ...
        logger.info(f"User {user_id} completed {requests_made} requests in {duration:.2f}s")
    ...
    def select_endpoint(self) -> Dict[str, Any]:
        ...
        usage_distribution = [
            ...
            (0.15, {"path": "/api/ehr/medical-records/", "method": "GET", "think_time": 3.0}),
            (0.1, {"path": "/api/lab/results/", "method": "GET", "think_time": 2.5}),
            (0.1, {"path": "/api/patients/", "method": "POST", "payload": {
                "first_name": "Test",
                "last_name": f"User_{int(time.time())}",
                "date_of_birth": "1990-01-01",
                "gender": "MALE"
            }, "think_time": 1.0}),
            (0.1, {"path": "/api/appointments/", "method": "POST", "payload": {
                "patient_id": 1,
            ...
            (0.05, {"path": "/api/analytics/dashboard/", "method": "GET", "think_time": 5.0})
        ]
    ...
    def analyze_results(self):
        ...
        logger.info(f"  Failed Requests: {failed_requests} ({failed_requests/total_requests*100:.1f}%)")
        logger.info(f"  Total Test Duration: {self.results[-1].timestamp - self.results[0].timestamp}")
        ...
        if response_times:
            ...
            logger.info(f"  99th Percentile: {statistics.quantiles(response_times, n=100)[98]:.3f}s")
            ...
            logger.info(f"\nPerformance Targets:")
            logger.info(f"  Average Response Time: {avg_response:.3f}s {'✓' if avg_response < 0.1 else '✗'} (Target: <100ms)")
            logger.info(f"  95th Percentile: {p95_response:.3f}s {'✓' if p95_response < 0.5 else '✗'} (Target: <500ms)")
            logger.info(f"  Success Rate: {successful_requests/total_requests*100:.1f}% {'✓' if successful_requests/total_requests > 0.99 else '✗'} (Target: >99%)")
        ...
        for endpoint, stats in endpoint_stats.items():
            if stats['response_times']:
                ...
                logger.info(f"    Success Rate: {stats['successful']/stats['requests']*100:.1f}%")
                logger.info(f"    Avg Response: {avg_time:.3f}s")

        # Throughput
        if response_times:
            total_duration = (self.results[-1].timestamp - self.results[0].timestamp).total_seconds()
    ...
    def save_results(self, filename: str = None):
        ...
        with open(filename, 'w', newline='') as csvfile:
            fieldnames = ['timestamp', 'endpoint', 'method', 'status_code', 'response_time', 'error']
