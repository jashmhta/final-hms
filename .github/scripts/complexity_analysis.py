...
class ComplexityAnalyzer:
    ...
    def run_lizard_analysis(self) -> pd.DataFrame:
        try:
            result = subprocess.run(
                ["lizard", ".", "--csv"], capture_output=True, text=True, check=True
            )
    ...
    def calculate_maintainability_index(self, file_path: str) -> float:
        try:
            ...
            mi = (
                171
                - 5.2 * np.log(max(1, operators + operands))
                - 0.23 * np.log(max(1, loc))
            )
    ...
    def calculate_aggregate_metrics(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if results["high_complexity_functions"]:
            avg_complexity = np.mean(
                [f["complexity"] for f in results["high_complexity_functions"]]
            )
        ...
        for issue in results["maintainability_issues"]:
            ...
            maintainability_counts[severity] = (
                maintainability_counts.get(severity, 0) + 1
            )
    ...
    def generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        critical_functions = [
            f
            for f in results["high_complexity_functions"]
            if f["severity"] == "critical"
        ]
        if critical_functions:
            recommendations.append(
                f"🔴 CRITICAL: {len(critical_functions)} functions have critical complexity levels"
            )
        ...
        high_functions = [
            f for f in results["high_complexity_functions"] if f["severity"] == "high"
        ]
        if high_functions:
            recommendations.append(
                f"🟠 HIGH: {len(high_functions)} functions have high complexity levels"
            )
            recommendations.append(
        ...
        critical_maintainability = [
            i for i in results["maintainability_issues"] if i["severity"] == "critical"
        ]
        if critical_maintainability:
            recommendations.append(
                f"🔴 CRITICAL: {len(critical_maintainability)} files have critical maintainability issues"
            )
        ...
        if results["high_complexity_functions"]:
            ...
            recommendations.append(
                "   - Use automated complexity tools in CI/CD pipeline"
            )
            recommendations.append(
        ...
        if results["complexity_metrics"]["average_complexity"] > 20:
            recommendations.append(
                "⚠️  High average complexity detected - consider architectural refactoring"
            )
    ...
    def generate_html_report(self, results: Dict[str, Any]) -> str:
        ...
        <body>
            ...
            <div>{distribution_rows}</div>
            <div>{complexity_rows}</div>
        </body>
        </html>
        """
        ...
        def generate_complexity_rows(functions):
            ...
            for func in functions[:20]:
                rows += f"<tr><td>{func}</td></tr>"
            return rows

        def generate_maintainability_rows(issues):
            rows = ""
            for issue in issues[:20]:
                rows += f"<tr><td>{issue}</td></tr>"
            return rows
        ...
        return html_template.format(
            ...
            complexity_distribution_rows=generate_distribution_rows(
                metrics["complexity_distribution"]
            ),
            maintainability_distribution_rows=generate_distribution_rows(
                metrics["maintainability_distribution"]
            ),
            complexity_function_rows=generate_complexity_rows(
                results["high_complexity_functions"]
            ),
            maintainability_issue_rows=generate_maintainability_rows(
                results["maintainability_issues"]
            ),
    ...
    def run_analysis(self):
        ...
        if results["high_complexity_functions"]:
            ...
            for i, func in enumerate(sorted_functions[:5], 1):
                print(
                    f"  {i}. {func['function']} in {func['file']} (complexity: {func['complexity']})"
                )
        print(f"\n📝 Recommendations:")
...
def main():
    ...
    critical_functions = [
        f for f in results["high_complexity_functions"] if f["severity"] == "critical"
    ]
    critical_maintainability = [
        i for i in results["maintainability_issues"] if i["severity"] == "critical"
    ]
