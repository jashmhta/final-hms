...
class TechnicalDebtAnalyzer:
                ...
                for block in blocks:
                    ...
                    if code_hash in code_blocks:
                        duplication_results['duplicated_blocks'].append({
                            ...
                            'similarity': self.calculate_similarity(normalized_code, code_blocks[code_hash]['normalized'])
                        })
                    else:
        ...
        if duplication_results['duplicated_blocks']:
            ...
            for block in duplication_results['duplicated_blocks']:
                for file_path in block['files']:
                    file_duplication_count[file_path] = file_duplication_count.get(file_path, 0) + 1
    ...
    def extract_code_blocks(self, lines: List[str], file_path: str) -> List[Dict[str, Any]]:
        blocks = []
        current_block = []
        current_line_num = 0
                ...
                if file.endswith('.py'):
                    ...
                    try:
                        ...
                        for line_num, line in enumerate(lines, 1):
                            if re.search(self.debt_indicators['todo_comments'], line, re.IGNORECASE):
                                ...
                                todo_info = {
                                    ...
                                    'type': todo_type,
                                    'text': line.strip(),
                                    'age_days': self.estimate_todo_age(file_path, line_num)
                                }
                                if todo_info['age_days'] is not None:
                                    todo_results['todos_by_age'][todo_type] = \
                                        todo_results['todos_by_age'].get(todo_type, []) + [todo_info['age_days']]
        ...
        try:
            ...
            for commit in commits:
                try:
                    ...
                    if line_num <= len(lines) and \
                       re.search(self.debt_indicators['todo_comments'], lines[line_num - 1], re.IGNORECASE):
    ...
    def analyze_file_code_smells(self, file_path: str, content: str) -> List[Dict[str, Any]]:
        smells = []
        lines = content.split('\n')
    ...
    def analyze_test_coverage(self) -> Dict[str, Any]:
        ...
        if coverage_results['python_files'] > 0:
            main_files = coverage_results['python_files'] - coverage_results['test_files']
            tested_files = main_files - len(coverage_results['untested_files'])
            coverage_results['coverage_estimate'] = (tested_files / main_files) * 100 if main_files > 0 else 0
        return coverage_results
    def is_main_module_file(self, file_path: str) -> bool:
        filename = os.path.basename(file_path)
        return not (filename.startswith('test_') or filename == '__init__.py' or 'test' in filename.lower())
    ...
    def calculate_debt_score(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        debt_scores = {}
        dup_score = min(100, analysis_results['duplication']['duplication_percentage'] * 2)
        ...
        return {
            ...
            'recommendations': self.generate_debt_recommendations(debt_scores, analysis_results)
        }
    def get_debt_level(self, score: float) -> str:
        if score >= 70:
            ...
            return 'low'
    def generate_debt_recommendations(self, scores: Dict[str, float], results: Dict[str, Any]) -> List[str]:
        recommendations = []
        sorted_scores = sorted(scores.items(), key=lambda x: x[1], reverse=True)
        for category, score in sorted_scores:
            if score >= 50:
                if category == 'code_duplication':
                    recommendations.append(f"🔴 High code duplication ({results['duplication']['duplication_percentage']:.1f}%)")
                    recommendations.append("   - Extract common code into shared modules")
                    recommendations.append("   - Use inheritance or composition to reduce duplication")
                elif category == 'todo_comments':
                    recommendations.append(f"🔴 {results['todo_comments']['total_todos']} TODO/FIXME comments found")
                    recommendations.append("   - Address TODO comments, starting with oldest")
                    recommendations.append("   - Implement a TODO tracking system")
                elif category == 'code_smells':
                    recommendations.append(f"🔴 {results['code_smells']['total_smells']} code smells detected")
                    recommendations.append("   - Refactor code smells systematically")
                    recommendations.append("   - Use static analysis tools to prevent new smells")
                elif category == 'test_coverage':
                    recommendations.append(f"🔴 Low test coverage ({results['test_coverage']['coverage_estimate']:.1f}%)")
    ...
    def generate_debt_report(self, results: Dict[str, Any]) -> str:
        ...
        report = {
            ...
            'summary': {
                ...
                'duplication_percentage': results['duplication']['duplication_percentage'],
    ...
    def generate_html_report(self, report: Dict[str, Any]) -> str:
        ...
        def generate_category_score_rows(scores):
            ...
            for category, score in scores.items():
                ...
                status = 'Critical' if score >= 70 else 'High' if score >= 50 else 'Medium' if score >= 30 else 'Good'
                rows += f
            return rows
        def generate_todo_rows(todos):
            rows = ""
            for todo in todos[:10]:
                age_text = f"{todo.get('age_days', 'N/A')} days" if todo.get('age_days') is not None else 'Unknown'
                rows += f
            return rows
        ...
        return html_template.format(
            ...
            category_score_rows=generate_category_score_rows(debt_score['category_scores']),
            todo_rows=generate_todo_rows(report['analysis_results']['todo_comments']['oldest_todos']),
            duplication_rows=generate_duplication_rows(report['analysis_results']['duplication']['most_duplicated_files']),
            recommendation_items=generate_recommendation_items(debt_score['recommendations'])
        )
    def run_analysis(self):
        ...
        print(f"Code duplication: {results['duplication']['duplication_percentage']:.1f}%")
