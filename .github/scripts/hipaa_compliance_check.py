...
class HIPAAComplianceChecker:
        ...
        for root, dirs, files in os.walk('.'):
            ...
            for file in files:
                ...
                try:
                    ...
                    if mode & 0o004 and any(sensitive in file.lower() for sensitive in
                ...
                if file.endswith(('.py', '.js', '.ts')):
                    ...
                    try:
                        ...
                        if any(keyword in content.lower() for keyword in encryption_keywords):
                            if 'key' not in content.lower() or 'generate' not in content.lower():
                                violations.append({
            ...
            for file in files:
                if file.endswith('.py'):
                    ...
                    try:
                        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                            content = f.read()
                            if ('import logging' in content or 'from logging import' in content) and \
                               any(keyword in content.lower() for keyword in ['audit', 'security', 'access']):
                                logging_found = True
                                break
                    except (UnicodeDecodeError, PermissionError):
                        continue
    ...
    def generate_recommendations(self, violations: List[Dict]) -> List[str]:
        ...
        if critical_issues:
            recommendations.append("🔴 CRITICAL: Address all critical severity violations immediately")
        high_issues = [v for v in violations if v['severity'] == 'high']
        if high_issues:
            recommendations.append("🟠 HIGH: Address high severity violations within 30 days")
        medium_issues = [v for v in violations if v['severity'] == 'medium']
        if medium_issues:
            recommendations.append("🟡 MEDIUM: Address medium severity violations within 90 days")
        violation_types = set([v['type'] for v in violations])
        if 'hardcoded_credentials' in violation_types:
            recommendations.append("🔑 Implement proper secret management and remove hardcoded credentials")
        if 'phi_detection' in violation_types:
            recommendations.append("🏥 Remove or properly mask all PHI from code and documentation")
