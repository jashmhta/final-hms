...
def enhance_security_headers():
    ...
    security_middleware_file = Path(__file__).parent / "backend" / "core" / "security_middleware.py"
...
class SecurityValidator:
    ...
    def validate_input(cls, value, field_type='text'):
        ...
        for pattern in cls.SQL_INJECTION_PATTERNS:
            if re.search(pattern, value_str, re.IGNORECASE):
                raise ValidationError(
                    _('Potential SQL injection detected'),
                    code='sql_injection'
                )
        ...
        for pattern in cls.XSS_PATTERNS:
            if re.search(pattern, value_str, re.IGNORECASE):
                raise ValidationError(
                    _('Potential XSS attack detected'),
                    code='xss'
                )
        ...
        for pattern in cls.PATH_TRAVERSAL_PATTERNS:
            if re.search(pattern, value_str, re.IGNORECASE):
                raise ValidationError(
                    _('Potential path traversal attack'),
                    code='path_traversal'
                )
    ...
    def validate_phone(phone):
        ...
        if not re.match(r'^\+?[\d]{10,15}$', phone_clean):
            raise ValidationError(_('Invalid phone number format'), code='invalid_phone')
